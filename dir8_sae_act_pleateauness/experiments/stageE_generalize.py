"""Direction 8 — Stage E: alternate-layer generalization of the Stage B null.

The project null (Stages A/B/B-dir/C/D) is established at resid_pre@6 (block-5 output).
The one remaining scope caveat is "one layer/SAE". Stage E reruns the DECISIVE Stage B
distance-matched test at a different layer with its own matching jbloom SAE, to check
whether the core finding generalizes:

  * recon lies ~ON the iso_displace random-displacement plateau(distance) curve
    (its plateau = closeness-to-real, no extra SAE credit), and
  * naive / sparse_match lie BELOW that curve (an SAE independent composition is LESS
    flat than a random point at equal distance),
  * i.e. NO SAE-decoded condition plateaus ABOVE random displacement at matched distance.

Default alternate layer: 9 (blocks.9.hook_resid_pre = output of block 8), a later layer
than the primary block-5/resid_pre@6. This is a direct copy of stageB_distance.py with the
layer/SAE parametrized; all logic (held-out tau, iso reference, distance-matched residual,
bootstrap CIs) is identical for an apples-to-apples comparison to Stage B.
"""
...
def main():
    ...
    ap.add_argument("--layer", type=int, default=9, help="resid_pre layer L; hook = block L-1")
    args = ap.parse_args()
    LAYER = args.layer
    SAE_PRE = f"blocks.{LAYER}.hook_resid_pre"
    HOOK_BLOCK = LAYER - 1
    ...
    print(f"[{time.time()-t0:.0f}s] layer resid_pre@{LAYER} (hook block {HOOK_BLOCK}); N={N}", flush=True)
    ...
    # ---- capture real resid_pre@LAYER last-token activations ----
    cap = {}
    ...
    # ---- establish b_dec convention empirically (as Stage A did for layer 6) ----
    def recon_err(sub):
        z = sae_encode(Xr, sae, sub)
        xh = sae_decode(z, sae)
        return float((xh - Xr).norm(dim=1).mean())
    err_sub, err_nosub = recon_err(True), recon_err(False)
    SUB = err_sub <= err_nosub
    print(f"[{time.time()-t0:.0f}s] recon err sub_bdec={err_sub:.1f} nosub={err_nosub:.1f} -> SUB={SUB}", flush=True)
    ...
    # naive: independent composition (k~empirical L0, idx~freq, coef~empirical)
    ...
    # sparse_match: k = source's OWN L0, coefs rescaled to source coef-RMS
    ...
    # ---- held-out tau: calibrate on real sources < N//2; score on >= N//2 ----
    split = N // 2
    calib = np.arange(N) < split
    evalm = ~calib
    r02 = R_GRID.index(0.02)
    tau = float(np.median(curves["real"][calib, :, r02].mean(1)))
    print(f"[{time.time()-t0:.0f}s] tau(held-out real calib)={tau:.6f}", flush=True)
    ...
    pdst = np.concatenate([dist[c][evalm] for c in pool_c])
    sp_pd = round(spearman(pa, pdst), 3)

    # raw paired gap vs real (eval half), reproduces the Stage-A ordering at this layer
    ...
    summary = {"stage": "E", "layer": f"resid_pre@{LAYER}", "hook_block": HOOK_BLOCK,
               "sae_pre": SAE_PRE, "d_sae": int(sae["W_dec"].shape[0]),
               "recon_err_sub": round(err_sub, 2), "recon_err_nosub": round(err_nosub, 2),
               "N": N, "n_eval": int(evalm.sum()), "n_dirs": N_DIRS, "r_grid": R_GRID,
               "tau_heldout": tau, "sub_bdec": bool(SUB), "iso_deltas": ISO_DELTAS,
               ...
               "groups_eval": {c: {"plateau_median": round(float(np.median(aucs[c][evalm])), 5),
                                   ...
                                   "norm_median": round(float(np.median(np.linalg.norm(conditions[c], axis=1))), 2),
    ...
    with open(os.path.join(RES, f"stageE_L{LAYER}_summary.json"), "w") as f:
        json.dump(summary, f, indent=2)

    with open(os.path.join(RES, f"stageE_L{LAYER}_metrics.csv"), "w", newline="") as f:
        wr = csv.writer(f)
    ...
    ax[0].set_title(f"resid_pre@{LAYER}: plateau vs distance (N_eval={int(evalm.sum())})")
    ...
    ax[1].bar(cs, meds, yerr=[los, his], capsize=6, color=[colmap[c] for c in cs], edgecolor="k")
    ax[1].axhline(0, c="k", lw=1)
    ax[1].set_ylabel("plateau residual vs random displacement\n(at matched distance, 95% CI)")
    ax[1].set_title(f"resid_pre@{LAYER}: distance-matched residual\n(>0 = flatter than random at equal distance)")
    plt.tight_layout(); plt.savefig(os.path.join(PLOTS, f"plateau_stageE_L{LAYER}.png"), dpi=110); plt.close()
