...
def run_pair(model, ids_a, ids_b, layer=0, valid=None, grid=GRID):
    ...
    return dict(d=d, w=width(d, grid), cos=cos, fallback=fallback, mv=step_jsd(z),
...
def step_jsd(z):
    """Movement profile m_j = JSD(q(t_j), q(t_{j+1})) in bits, for the 49 neighbouring positions.

    q(t) = softmax over the valid vocabulary of the patched final-position logits, so this is how
    far the model's FULL next-token distribution actually moves at each step along the path, not
    just the relative-logit-distance summary d(t). Computed from log-softmax for stability and
    returned as a length-49 float array; the full |valid|-wide probabilities are never saved.
    """
    lp = torch.log_softmax(z, dim=-1)                        # natural log q(t), shape (T, |valid|)
    a, b = lp[:-1], lp[1:]
    lm = torch.logaddexp(a, b) - float(np.log(2.0))          # log of midpoint (q_j + q_{j+1}) / 2
    nats = 0.5 * ((a.exp() * (a - lm)).sum(1) + (b.exp() * (b - lm)).sum(1))
    return (nats / float(np.log(2.0))).clamp_min(0.0).cpu().numpy()
