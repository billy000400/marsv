# VENDORED VERBATIM from cupbearer (https://github.com/ejnnr/cupbearer), the PyTorch build,
# src/cupbearer/detectors/statistical/helpers.py (downloaded main, 2026-06-21).
# We vendor only this self-contained pure-torch math module so we can use cupbearer's actual
# detector algorithms (relative-Mahalanobis distance + Quantum-Entropy / SPECTRE) as baselines
# WITHOUT pip-installing the package: its pyproject pins numpy>=1.24,<2 (conflicts with the shared
# numpy 2.3.3 that must not be changed) and pulls heavy lightning/torchvision/torchattacks deps.
# See experiments/ENV_NOTES.md for the compatibility finding. Nothing here is modified.
