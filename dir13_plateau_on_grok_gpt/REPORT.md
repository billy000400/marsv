...
The last explanation left for the remaining gap — that all these rules score units one at a time and
miss units that only matter jointly — is measured and turns out to be small. Rebuilding the set in
eight rounds, re-measuring each unit's importance with the units already chosen straightened, gains
**3.4 points at $k=128$** (68.4% → **71.8%**, $p=6.1\times10^{-21}$, 84.7% of pairs not worse) and
nothing at $k=32$ (50.9% → 49.8%, $p=0.43$). Joint effects therefore close about a fifth of the
distance from the best one-shot ranking to the all-units ceiling, and only in the tail: the leading
units carry the bend nearly independently, and it is the interchangeable remainder a sequential rule
can reallocate.
Cutting that ranking into six bands and linearizing each band **on its own** then changes how the whole
top-$k$ curve should be read. Every band except the bottom one straightens between a tenth and a quarter
of the path by itself, and the six band-alone effects sum to **111.5%** against the 85.2% all-units
ceiling, so the same bend is carried in several places at once and a nested-prefix curve makes the
mechanism look more concentrated than it is. The ordering keeps predicting causal effect down to about
rank 2,048 — each band beats a same-size random draw from its own region of the ranking,
$p\le10^{-25}$ — and predicts nothing below it, where the last 1,792 units bend the path not at all.
Across five checkpoints the redundancy is already there at the first one (1.21, 1.01, 1.08, 1.21, 1.18),
so it is present as soon as there is a bend to share rather than accumulating; a second training run
repeats the whole decomposition — every band alone within 2.5 points, the same roughly 600-fold decline
in per-unit worth, and a ratio above 1 at every checkpoint — and differs only in drifting up slightly
over training where the reference run ends where it began. What training builds is a head whose membership it keeps rewriting — at
step 831 a pair's top eight units already remove 7.2% of its gap, and **none** of them is a unit the
finished network ranks in that pair's top eight. Those eventual head units are promoted from just below
the head: at step 831 they already sit at median rank **113.5** of 3,840 (a random unit sits at 1,919.5)
with half of them inside that checkpoint's top 128, and they climb smoothly from there. What those units
compute changes in step with that turnover: refitting the character-window probe at every
...
(**0.92 → 0.41**) — read through that probe, character readability drains away with head membership
rather than marking a fixed set of units. Giving the probe 32 characters of history and the eight
characters that follow the position leaves the drain in place at about a quarter of its size, so its
size is already a fact about the probe's feature set. What the units moved *to* is still open: adding where the
position sits in its line and word, which word and which three-character string it is inside, and the
character composition of the earlier text closes only **9%** of what the character families miss on
those units and leaves the fall unmoved at −0.064, so nine feature families bound the change without
naming it. A large part of what they miss turns out not to be new material but a new *shape*: a one-hidden-layer
read-out of the same 40 characters lifts the demoted units from **0.78** to **0.92** at step 30,000 and
reverses the network-wide decline outright (−0.011 under the linear probe, **+0.019** under the nonlinear
one). Doubling that read-out's width, or adding a second hidden layer, removes the demoted units' own
fall as well (−0.064 → −0.026 → **−0.005** and **−0.004**), so the decline belongs to the probe rather
than to the units; what is left is the level, 4.8% of a demoted unit's response still unexplained at step
30,000 by the largest probe fitted here. Training the same recipe a second time (model seed 2024) reproduces the turnover, the
...
whose boundaries coincide with next-character prediction changes** — a coincidence, not an identity:
the 65 endpoint characters produce only 15 distinct endpoint predictions, the median path visits three
predictions rather than two, and on 9.9% of pairs both endpoints share one prediction, so decisions
cannot label basins one-to-one. That description is also read off one token slot rather than being the
phenomenon: the sharp switch is still there several characters downstream where the decision is not,
and its mechanism sits upstream in the early MLPs. Qualified further because we tested a reconstruction rather than the
...
boundary; the measured invariance of $d(t)$ and the size of the shift decide it (Figure 23).
...
$\Delta w = w^{g} - w^{g=1}$ and $\Delta t^{*} = t^{*g} - t^{*g=1}$ on the same pairs (Figure 24). The
...
a surviving decision alongside a straight $d(t)$ falsifies that reading (Figure 25).
...
other outcome falsifies them (Figure 26).
...
*its* matched-accuracy checkpoint, so neither training length nor task performance differs (Figure 27).
...
frozen-mirror seed (Figures 27 and 28).
...
compared with the recorded top-32 of Figure 30. The primary score is **differential tuning** — a unit
...
**Non-character probe families** — the check that asks what the leftover response *is*. Even the widest
character family leaves about a fifth of a demoted unit's response unexplained, and there are two
obvious candidates for it: text further away than the probe can see, and structure that is not the
identity of any single character (where the position sits in its line or word, which word it is inside).
Three further families are therefore fitted with the same estimator, the same $\lambda$ grid, the same
80/10/10 split and the same rows. Writing $e(\cdot)$ for the one-hot code of a categorical feature of
position $p$:

```math
\text{struct}:\ \beta_{0,j}+\sum_{f\in\lbrace \text{col},\ \text{wpos},\ \text{prevlen},\ \text{nextsep},\ \text{caps}\rbrace}\delta_{f,j}^{\top}e\bigl(f(p)\bigr),
```

```math
\text{lex}:\ \beta_{0,j}+\sum_{f\in\lbrace \text{word},\ \text{prevword},\ \text{tri}\rbrace}\eta_{f,j}^{\top}e\bigl(f(p)\bigr),
\qquad
\text{bag}:\ \beta_{0,j}+\theta_{j}^{\top}c(p)+\phi_j\, m(p).
```

The five structural features are: $\text{col}$, the number of characters since the last newline (capped
at 40, with one bucket for "no newline inside the window"); $\text{wpos}$, the number since the last
space or newline (capped at 12); $\text{prevlen}$, the length of the previous completed word (capped at
12); $\text{nextsep}$, the distance to the next space or newline within the following eight characters
(one bucket for "none"); and $\text{caps}$, the length of the run of capital letters ending at the
position (capped at 4). None of them names a character. The three lexical features are the identity of
the partial word ending at the position ($\text{word}$), of the previous completed word
($\text{prevword}$) — each the 255 most frequent strings in the corpus plus one "other" bucket — and of
the three-character string ending at the position ($\text{tri}$, the 511 most frequent plus "other").
These are *conjunctions* of characters, which a sum of per-position character effects cannot represent.
In the bag family $c(p)\in\mathbb{R}^{65}$ holds how often each character of the vocabulary occurs in
the part of the window lying before lag 32, divided by the length of that stretch, and $m(p)$ is that
length divided by 128; it is the only family here that reaches past all40's 40-character neighbourhood
(the stretch runs from 1 to 88 characters, median 44.5). Two combinations complete the set: **nonchar**,
the three new families together, and **union**, nonchar plus all40 — every feature this report fits.
char1, past8 and all40 are refitted inside the same design so that the comparison is unit by unit; they
reproduce Figure 44's fits exactly (largest per-unit difference over 3,840 units: 0 for char1 and past8,
$5\times10^{-13}$ for all40). Consumed in Figure 45.

**A nonlinear read-out of the same characters** — the check that asks whether the leftover response is a
nonlinear *function* of characters the design already holds. Every family above is fitted by ridge and is
therefore linear in its one-hot design, so all of them share one blind spot: a unit that responds to a
combination the design does not spell out — an apostrophe, say, but only inside a capitalised word —
scores low under every one of them however wide the window. The last probe keeps all40's design and
changes the estimator. For unit $j$ at position $p$:

```math
\hat y_j(p)=w_j^{\top}\,\mathrm{gelu}\Bigl(\sum_{g=1}^{42} E_{\,\mathrm{code}_g(p)}\Bigr)+b_j ,
```

where $g$ runs over all40's 42 one-hot groups (the intercept, the characters at lags 0 to 31, the eight
characters after the position, and the lag0 $\times$ lag1 table), $\mathrm{code}_g(p)$ is the column that
group puts a 1 in, $E$ holds one 768-dimensional row per design column, and $\mathrm{gelu}$ is the
nonlinearity the model's own MLP uses. A sum of embeddings is itself a linear map of the one-hot design,
so deleting the $\mathrm{gelu}$ turns this into the **linear control**: same parameters, optimiser,
epochs and stopping rule, differing only in the nonlinearity, and expressive enough to represent any
linear map of the design up to a rank-768 factorization. The gap between the two is what the
nonlinearity is worth; the gap between the control and the published ridge fit says whether the new
estimator reproduces the old one at all. Both are trained by Adam (learning rate $10^{-3}$, batches of
4,096 positions) on the same 80% train split, with the epoch and the weight decay (0 or $10^{-4}$) chosen
by median validation $R^2$ — the role the $\lambda$ grid plays for ridge — and training stops once
validation $R^2$ has not improved for 20 epochs. $R^2$ is reported on the same held-out test split as
every other family. Storing one checkpoint's activations for all 3,840 units at once exceeds the memory
budget, so this probe is fitted for 797 of them: all 75 stable, 141 demoted and 181 promoted units, and a
...
### Do the units interact? Residual-corrected (greedy) selection

Every rule above scores each unit on its own and keeps the top $k$. Such a rule cannot notice that two
units carry the same piece of bend, nor that a unit only matters once another has been straightened —
and "joint effects" was the standing explanation for the gap between the best rule and the ceiling
reached by linearizing all 3,840 units. To measure that explanation instead of assuming it, selection
stays fitted (it may read the pair's own activations) but becomes **sequential**: the set is built in
$R$ equal rounds, and before each round the importance of every unit is re-measured *with the units
already chosen linearized*, so a unit is scored by the bend that is still left for it to carry.
Writing $S_r$ for the set after round $r$ and $a^{S}_j(t)$ for unit $j$'s recorded activation when the
units in $S$ are chord-substituted, the **residual importance** is:

```math
I^{S}_j=n_j\cdot\max_t\bigl\lvert a^{S}_j(t)-\mathrm{chord}_j(t)\bigr\rvert ,
\qquad S_{r+1}=S_r\cup\lbrace\text{the }k/R\text{ units }j\notin S_r\text{ with largest }I^{S_r}_j\rbrace ,
```

with $\mathrm{chord}_j(t)=(1-t)a_j(0)+t\,a_j(1)$ as before and $S_0=\varnothing$. Units already in
$S_r$ have $I^{S_r}_j=0$ by construction, so the rule never re-picks one. Because the chord matches the
true activation at both endpoints, linearizing $S_r$ leaves $a_j(0)$ and $a_j(1)$ — and hence every
chord — unchanged, so the rounds differ only in what happens between the endpoints.

$R=1$ is exactly the one-shot pair-fitted rule $I_j$ of the earlier section, which makes it both the
control and a free reproduction check: the sole difference as $R$ grows is that later rounds see the
network after the earlier picks are straightened, so any gain is a joint effect and nothing else. We
run $R\in\lbrace 1,2,4,8\rbrace$ at $k\in\lbrace 32,128\rbrace$ on the same 150 pairs, score with the
same recovered fraction $\rho(S)$, and additionally report the **set overlap**
$\lvert S^{R}\cap S^{1}\rvert$ — how many of the $k$ picks the sequential rule keeps — so a change in
$\rho$ can be read against how much of the set actually moved. This is a batched approximation to full greedy search:
exhaustive greedy would add one unit at a time and score each candidate by its measured effect on
$d(t)$, costing about $k$ times more forward passes, so the gain reported here is a lower bound on
what joint selection can achieve.

### Band decomposition: why the last third of the bend costs hundreds of units

The recovered-fraction curve $\rho(k)$ climbs steeply and then crawls: the top 32 units remove about
half of the trained-to-untrained width gap and the remaining thousands remove the rest. Two very
different pictures produce that shape. Either the units far down the ranking each carry a small amount
of bend of the same kind as the leaders, or they are a different kind of unit that only bends a path in
company. Nested prefixes cannot tell these apart, because every prefix contains the leaders. Measuring
each slice of the ranking **on its own** can.

Cut the per-pair importance ranking $I_j$ into six **bands** at rank edges
$e=(0,8,32,128,512,2048,3840)$, so band $B_b$ holds the units ranked from $e_b$ up to $e_{b+1}$ (sizes
8, 24, 96, 384, 1,536, 1,792). Each band is chord-linearized alone and scored with the same recovered
fraction $\rho$. Three comparisons are run alongside it.

**Marginal contribution inside the nested prefix** — what the same band adds when the units above it
are already linearized, with $P_b$ the prefix of everything ranked above $e_{b+1}$:

```math
\Delta_b=\rho(P_b)-\rho(P_{b-1}),\qquad P_b=\lbrace j:\ \mathrm{rank}_j<e_{b+1}\rbrace .
```

A band that beats its own marginal contribution is carrying bend that the units above it also carry.

**Redundancy ratio** — the same statement for the ranking as a whole, computed per pair and then
summarized by its median over pairs:

```math
\Lambda=\frac{\sum_{b=1}^{6}\rho(B_b)}{\rho(P_6)} .
```

$\Lambda=1$ means the six bands add up exactly, $\Lambda>1$ means they overlap in what they bend, and
$\Lambda<1$ would mean they need each other.

**Two size-matched random controls.** The first draws $|B_b|$ units uniformly from all 3,840, which
holds the edit budget fixed. That control is misleading for the lower bands on its own: a random draw
of 1,536 units already contains about 40% of the top 32, so it inherits the leaders' effect. The second
control therefore draws $|B_b|$ units uniformly from the units ranked at or below that band's own lower
edge, which asks whether the ordering carries information *within* the region the band lives in. Only
the second is used to claim signal.

Per-unit worth is reported as $1000\thinspace\rho(B_b)/|B_b|$, the percentage of the gap removed per thousand
units, so bands of very different sizes can be compared.

Here $\rho$ is computed **per pair and then medianed**, whereas the earlier sections form the ratio
from median widths. The prefix column therefore reproduces the earlier $\rho(k)$ curve to within about
1.5 points at every $k$ and serves as the internal check on this pipeline. Pairs whose
trained-to-untrained width gap is below 0.10 are dropped, because the per-pair ratio is unstable when
there is almost no gap to recover; all 150 pairs pass at step 30,000.

**Which band a unit belongs to.** The band decomposition ranks units per pair, so describing "the tail"
as a population needs one band label per unit. Each unit is assigned to the band containing its **best
(minimum) rank over the 150 pairs**: a head unit is one that at least one pair ranks highly, a tail unit
is one that no pair does. Assigning by *mean* rank fails here — a unit that matters intensely for a
single pair sits mid-table on average, which leaves the head band empty. Each band's units are then
described by the fitted character-window probe above, using its held-out $R^2$ for the full description
and for the current character alone ($L=1$). Head and tail are compared with a Mann–Whitney test over
**distinct units**, so a unit that appears in many pairs' rankings still counts once.

**When the head is chosen.** The decomposition is repeated at checkpoints 831, 2,038, 5,000, 12,500 and
30,000 of the same run. Fewer pairs clear the 0.10 gap filter early (94, 140, 146, 148, 150), so the
developmental trend is read on the **94 pairs usable at every checkpoint**. Separately, and with no
ablations at all, each checkpoint's per-pair top-$k$ set is compared with the same pair's step-30,000
top-$k$ set for $k\in\lbrace 8,32\rbrace$; the overlap expected by chance is $k^2/3840$ units (0.02 at
$k=8$, 0.27 at $k=32$). Overlap between consecutive checkpoints says whether membership settles early
or keeps changing.

**Where the final head units come from.** An overlap of zero says the finished network's head units were
not in the head early; it does not say where they were, and two developmental stories fit it. They might
already be near the top of the ranking and climb slot by slot, or they might sit anonymously in the
middle until late training and jump in. Rank separates these. The same recording pass gives every unit's
rank (0 = most important, 3,839 = least, so a uniformly drawn unit sits at 1,919.5 in expectation), so
for each pair we read the rank of its **step-30,000 top-8 units at every earlier checkpoint**, and the
mirror trajectory of its **step-831 top-8 units at every later one**, which says whether displaced units
fall to anonymity or stop just below the head. We also report the fraction of each pair's final top-8
that is already inside the top 8, 32, 128 and 512 at each checkpoint.

**Whether the promoted units are describable.** Ranks say *when* a unit joins the head, not what it
computes. To ask whether training promotes a particular kind of unit, every unit is labelled by its role
at the two ends of training: **promoted** if some pair ranks it in its top 8 at step 30,000 and no pair
did at step 831, **demoted** if the reverse, **stable** if both, and **never-head** if neither. Each
group is then described by the same fitted character-window probe, using held-out $R^2$ for the full
description and for the current character alone ($L=1$). Groups are compared with a two-sided
Mann–Whitney test over distinct units and reported with the **common-language effect size**
$U/(n_1 n_2)$ — the chance that a unit drawn from the first group scores above one drawn from the
second, 0.5 meaning no difference — alongside each group's median $R^2$.

Comparing promoted with demoted units on their own confounds the labels with present importance: a
promoted unit's best rank at step 30,000 is at most 7 and a demoted unit's is at least 8, and best rank
is itself associated with describability (Figure 37c). The comparison is therefore repeated **inside a
band of best rank**, where both groups share a level of present importance. Within ranks 0–7 (the
finished head) stable units are compared with promoted ones; within ranks 8–31 (just below the head)
demoted units are compared with the units of the same rank that were never in either head. All of these
tests read probe $R^2$ measured once, on the step-30,000 model, so they compare the finished network's
units labelled by their early role.

**Describability at the checkpoint where the unit held the head.** One model cannot separate two
readings of that comparison: training might keep a fixed set of describable units and add harder ones,
or a unit might be describable exactly while it holds the head. The same probe is therefore refitted
from scratch at each of the five checkpoints — same corpus, same window, same splits, same $\lambda$
grid — which gives every unit an $R^2$ trajectory across training and, between the ends, a per-unit
change. The whole-unit median at each checkpoint is the background those trajectories are read against,
since the network's overall character-readability need not be constant. Two contrasts also look
*forward* from step 831, using that checkpoint's own per-pair top-8 and top-32 sets for bands (band 0–7
holds the units some pair ranks in its top 8 there, band 8–31 the units some pair ranks in its top 32
but none in its top 8): inside the early head, the units that keep the head against those that lose it;
one band below, the units that will be promoted against those that never hold it. The step-30,000 refit
is the pipeline check — its per-unit $R^2$ must reproduce the published values.

**Are these developmental facts properties of the recipe or of one initialization?** Every measurement
above reads one training run, so a different initialization and data order could in principle produce a
different developmental story. We therefore train the reference recipe a second time with model seed
**2024** (the seed the other second-seed runs in this report use; data seed unchanged at 42, corpus,
architecture, optimizer, schedule, batch size and checkpoint grid all identical to the reference run)
and repeat all three measurements on the new run's checkpoints with the same code: the same 150 pairs,
the same context, the same block-0 interpolation, the same five checkpoints, the same importance score,
and the same probe refitted at each checkpoint. Roles (promoted / demoted / stable / never-head) are
defined inside each run from that run's own top-8 sets, since unit indices are not comparable across
initializations. The comparison to read is whether each measured trend has the same sign and rough size
in both runs; with two runs we can say a finding reproduces, not put a confidence interval on it.

The band decomposition itself is repeated on those same seed-2024 checkpoints, since it too was measured
on one run: the band-alone and all-units effects at all five checkpoints (which give $\Lambda$ at each
one), and at step 30,000 the same-size same-region random control, the per-unit worth, and the
character-window probe refitted on that run and read against the band of each unit's best rank. Three
parts of the seed-1337 decomposition are not repeated, because nothing in the second-seed comparison
turns on them: the marginal column $\Delta_b$ inside the nested prefix, the naive random draw from all
3,840 units, and the greedy re-selection.

Everything in this subsection except that second training run is forward passes on stored checkpoints,
and both endpoints stay exact to $10^{-6}$ under every edit.
...
are the primary evidence (Figures 6, 8, 19, 23).
...
A basin is a stretch of the path the model refuses to leave, and two different labels can be attached
to one: the **endpoint character** patched in, or the model's **argmax next-character prediction** at
that position. Since the transitions coincide with prediction changes (91% of changes fall inside the
transition window), it is tempting to merge the two and read the result as "one next-character
decision basin per character". Counting the predictions the paths actually visit shows the merge does
not hold. The counts use the argmax already stored for all 2,080 pairs, so no new forward passes were
needed (Figure 16).
...
**Figure 16.** Endpoint characters and decision classes are not the same labelling. Reference
character GPT at step 30,000, interpolation after block 0, all 2,080 character pairs, argmax
next-character prediction read at the patched position across the 50-step path. **(a)** x: number of
distinct argmax predictions visited along a path; y: percentage of the 2,080 pairs. The solid bar is
"exactly two", the count that a plateau–boundary–plateau story *in decision space* requires; hatched
bars are every other count. **(b)** x: the 15 distinct predictions produced by the 65 endpoint
characters (`␣` = space, `\n` = newline); y: how many endpoint characters map to that prediction; a
many-to-one bar means those characters are indistinguishable by their decision.

**The 65 endpoint characters produce only 15 distinct endpoint predictions.** The largest class
contains **13** characters and the next **11**; just 5 of the 15 predictions come from a single
character. A decision label cannot individuate the 59 characters that own a basin, so basins and
decision classes are not in one-to-one correspondence.

**The typical path visits three predictions, not two.** Exactly two are visited on **31.6%** of paths;
the median is **3** (interquartile range 2–3, maximum 7). Most paths therefore pass through at least
one intermediate decision belonging to neither endpoint, while the $d(t)$ curve still shows a single
plateau, boundary and plateau.

**On 9.9% of pairs both endpoints share one prediction.** For those the decision does not distinguish
the endpoints at all, yet the interpolation still rests near each end and switches sharply between
them — direct evidence that the resting behaviour is not the decision.

**What this narrows.** The basins are **character-conditioned and live in logit space**, and their
boundaries *coincide with* prediction changes rather than being defined by them. Every measurement in
this section — the rest-ratio criterion and its null families, the per-character basin fractions, the
frequency dependence, the 78% per-character variance share — is computed from the geometry of $d(t)$
and is unaffected. What does not follow is a one-to-one map between basins and next-character
decisions, so this report does not use that stronger phrasing. The 91% coincidence keeps its original,
weaker reading: where the path switches basins, the model's prediction usually changes too.
...
**Figure 17.** Basin ownership and transition sharpness move together across the vocabulary.
...
training frequency at Spearman $\rho = 0.56$ ($p = 1.0\times10^{-6}$, n = 65; Figure 18), and every
...
**Figure 18.** Basin ownership tracks how often the character appears in training. x: the number of
...
into every pairing it appears in. Figure 19 shows the raw curves behind this for six representative
...
same next character. Before that, Figure 20 answers the question it depends on — does the boundary sit
...
**Both mandatory controls are decisive** (Figure 22). *Learned, not architectural:* at initialization
...
**Figure 22.** Controls. Left: distribution of $w_{10\to90}$ (x) against number of pairs (y) for the
...
78% of the width variance explained by per-character terms alone (Figures 14–17), the structure is absent at
...
validation accuracy (Figure 26) — so the site is contingent, and what freezing costs is sharpness,
...
shared pool (Figure 30). Those units are character detectors: each unit's tuning measured in ordinary
...
puts the decision boundary exactly at the path midpoint. Figure 23 shows where the boundary lands.
...
**Figure 23.** Readout rebalancing on 1,873 character pairs, interpolation block 0, step 30000. Left
...
is the specificity control (Figure 24).
...
**Figure 24.** MLP-gain intervention, 150 character pairs, interpolation block 0, step-30000 fresh
...
**Figure 25.** Per-block MLP ablation, 150 character pairs, interpolation block 0, step-30000 fresh
...
of the same 30,000-step schedule. Figure 26 collects the outcome of all seven runs.
...
has to clear, so Figure 28 shows it beside the six gaps this section's conclusions rest on.
...
the forward computation and not geometry supplied by the prompt shape (Figure 29).
...
**Figure 29.** Readout offset sweep on the reference character GPT at step 30,000, 150 character pairs,
...
been using for it: a prediction flip is what the boundary looks like *at the patched position*, and four characters downstream the decision is gone while the discrete switch remains. What
...
contributions? Figure 30 gives the three readings that decide it.
...
**Figure 30.** Chord linearization of MLP hidden units in blocks 1–4, 150 character pairs,
...
To show that it does, Figure 31 puts the corpus-derived rankings beside the pair-blind controls, and
...
**Figure 31.** Corpus character tuning versus recruitment in the interpolation assay; reference
...
$\max(z_a,z_b)$; "global importance" is the assay-derived pair-blind ranking of Figure 30; "overall
...
Figure 30 is exactly the sharply tuned population: the 668 ever-recruited units have median sharpness
...
$I_j$, or anything else from the assay (Figure 32).
...
**Figure 32.** Chord linearization with units selected by corpus tuning alone; reference character GPT
...
the held-out corpus rule $|z_a-z_b|$; dashed/squares = the assay's own per-pair top-$k$ of Figure 30,
...
character pair. Figure 33 shows the descriptive half in panel (a), the direct comparison of the two
...
**Figure 33.** What the units missed by character tuning are, and whether previous-character
...
dashed line marks the current-character rule. Random and the fitted per-pair ceiling are Figure 32's
...
$R^2$ reported on another. Figure 34 shows the descriptive half in panel (a) and the causal half — a
...
intervention, the same 150 pairs, the same $k$ (Figure 35). Three predictions were registered before
...
**Figure 35.** Neither converting a score into residual-displacement units nor replacing the corpus
...
### Units interact only in the tail: joint selection is worth 3.4 points at $k=128$, nothing at $k=32$

The previous section ended by naming the one explanation still standing for the gap between the best
ranking and the all-units ceiling: every rule scores units individually and none can see that a set
works better than its members do apart. Sequential selection measures that explanation directly.
Building the set in $R$ rounds and re-measuring residual importance $I^{S_r}_j$ before each round
gives a rule that *does* see joint structure, while $R=1$ is the one-shot ranking itself, so the
comparison isolates the joint effect and nothing else (Figure 36). Three predictions were registered
before the run; the two that failed are the informative ones.
...
Reference character GPT at step 30,000, interpolation after block 0, blocks 1–4, 3,840 hidden units,
150 character pairs, top-$k$ units chord-linearized. **(a)** Recovered fraction $\rho(S)$ as a
percentage (y) against the number of selection rounds $R$ (x, log scale; $R=1$ is the one-shot
pair-fitted ranking $I_j$). Solid with circles = $k=32$, dashed with squares = $k=128$; the dotted
horizontal line at each set size marks the best per-unit rule measured anywhere in this report
($E_j$ at $k=32$, $D^{\mathrm{probe}}_j n_j$ at $k=128$), and the black dash-dotted line is the
ceiling reached by linearizing all 3,840 units. **(b)** Empirical cumulative distribution (y: fraction
of the 150 pairs) of the per-pair change in transition width $w$ between $R=8$ and $R=1$ (x; positive
= sequential selection removes more of the gap); the vertical line is no change. **(c)** Median
percentage of the $k$ selected units that the sequential rule keeps from the one-shot set (y) against
$R$ (x, log scale), same two series.

**At $k=32$ joint selection buys nothing.** Recovery reads 50.9% ($R=1$), 51.3%, 49.8%, 49.8%
($R=8$); none differs from the one-shot ranking (paired $p=0.24$, $0.41$, $0.43$), and just 50.7% of
pairs are not worse — a coin flip. Both predictions attached to this set size fail: four rounds were
registered to gain at least 5 points, and eight rounds to clear the 56.6% of the best per-unit rule.

**At $k=128$ it is real, monotone and broad.** Recovery rises 68.4% → 70.7% → 71.1% → **71.8%**, each
step significant against one round ($p=9.4\times10^{-17}$, $1.5\times10^{-19}$,
$6.1\times10^{-21}$), and even the final doubling $R=4\rightarrow8$ adds a significant amount
($p=6.0\times10^{-6}$). It is not a median artifact: **84.7%** of the 150 pairs are not worse under
eight rounds, with a median width gain of $+0.0145$.

**The gain is a small reallocation of the same set.** Eight rounds keeps a median **100 of 128** picks
at $k=128$ and **26 of 32** at $k=32$. Swapping about a fifth of the units is worth 3.4 points at the
larger size and nothing at the smaller one.

**Why this matters.** It closes the mechanism thread with a bounded number in place of an open
possibility. Joint effects among the units that carry a plateau boundary are real but small: they
recover **3.4 of the 18.3 points** — roughly a fifth — separating the best one-shot ranking from the
all-units ceiling at $k=128$, and none of the gap at $k=32$. The contrast between the two set sizes
locates them. The leading units carry the bend nearly independently, which is why scoring them one at
a time is already close to the best a size-32 set can do; the tail is nearly interchangeable, and
there a sequential rule can reallocate picks toward whichever units still bend the path once the
leaders are straight. This also corrects the reading the previous section carried forward: a blind
text-only rule beats the fitted per-pair ranking at $k=32$ *not* because that ranking is blind to
joint structure — a rule that sees joint structure perfectly does no better there — but because of the
form of the per-unit score, endpoint displacement against path curvature, which the previous section
had already isolated. For anyone using this machinery to find the units behind a new pair's geometry,
the recipe stays one line and now has an error bar on its refinement: rank by the activation swing the
endpoint swap causes, take the top $k$, and expect sequential re-selection to be worth a few points
only for sets in the hundreds.

**Caveats.** Rounds are equal-sized and stop at $R=8$ (16 units per round at $k=128$); exhaustive
greedy — one unit at a time, each candidate scored by its measured effect on $d(t)$ — is the stronger
test at roughly $k$ times the compute, so 3.4 points is a lower bound on what joint selection can
reach, not the maximum attainable by any 128 units. Two set sizes, 150 pairs, one context, one
checkpoint, one model, and the result speaks about this chord intervention rather than about unit
interactions in general. Free checks, all exact: $R=1$ reproduces the stored pair-fitted widths per
pair (max deviation 0.000000 at both $k$), the unmodified baseline reproduces to median 0.3507 (max
deviation 0.000000), and both endpoints stay exact under every selection (worst deviation $10^{-6}$).

### The tail is weak, redundant and continuous, and training keeps rewriting the head

The previous sections leave the shape of the ranking unexplained: 32 units remove about half of the
width gap and it takes thousands more to remove the rest. Cutting the ranking into six bands and
linearizing each band **alone** separates the two readings of that shape — individually weak units
versus units that only work in company — and the answer is neither (Figure 37).
...
less describable units. Reference character GPT at step 30,000, interpolation after block 0, blocks 1–4,
3,840 hidden units, 150 character pairs. **(a)** Percentage of the trained-to-untrained median width gap
removed, $\rho$ (y), against the importance-rank band (x; band size in the tick label). Solid blue with
`//` hatch = the band chord-linearized alone; vermillion with `\\` hatch = the same band's marginal
contribution $\Delta_b$ inside the nested prefix; pink dotted with `..` hatch = a random set of the same
size drawn from the same region. The black dash-dotted horizontal line is the ceiling reached by
linearizing all 3,840 units. **(b)** Per-unit worth, $1000\thinspace\rho(B_b)/|B_b|$ (y, log scale), against
band (x); circles = the ranked band, triangles (dashed) = the same-size same-region random control.
**(c)** Held-out $R^2$ of the fitted character-window probe (y) against the band of each unit's **best**
rank over the 150 pairs (x; number of units assigned in the tick label). Circles (solid) = the full
description (characters, context and interactions); squares (dashed) = the current character alone.
Shaded/hatched envelopes are the interquartile range over units.

**Redundant, not independent.** Each band alone removes 27.9%, 25.9%, 24.1%, 20.4%, 10.4% and −0.1% of
the gap, from the top band down. Those six numbers sum to **111.5%** against an all-units ceiling of
**85.2%** — a redundancy ratio of $\Lambda=1.29$ per pair (paired $p=6\times10^{-20}$, 86.7% of the 150
pairs above 1). The same statement holds band by band: every band alone exceeds its own marginal
contribution inside the nested prefix (25.9 vs 20.0%, 24.1 vs 17.9%, 20.4 vs 16.5%, 10.4 vs 2.7%; all
$p\le2\times10^{-8}$), and the excess grows down the ranking. So the bands overlap in what they bend.
A band far down the ranking can straighten a quarter of the path on its own, and then adds almost
nothing once the leaders are already straight.

**The ordering carries signal to rank 2,048 and none below it.** Every band except the last beats a
random set of its own size drawn from its own region of the ranking — 25.9 vs 0.15%, 24.1 vs 1.19%,
20.4 vs 2.24%, 10.4 vs 3.90%, each with $p\le10^{-25}$ and 98–100% of pairs — so a unit's rank predicts
its causal effect even among units nobody would call important. The last 1,792 units are the exception:
they remove −0.1% of the gap, indistinguishable from their own control, which for the bottom band *is*
the band. What falls off is the worth of a unit: 34.8% of the gap per thousand units in the top band,
0.067% in the 512–2,048 band, a roughly 500-fold decline (Figure 37b). The naive control that draws
from all 3,840 units recovers 40.6% and 48.2% for the two largest bands, more than the ranked bands
themselves, because a random 1,536-unit draw already contains about 40% of the top 32 — which is why
the within-region control is the one the claim rests on.

**A continuum of less describable units.** Sorting units by their best rank over the 150 pairs and
reading off the character-window probe gives a smooth decline across the six bands: held-out $R^2$ of
the full description 0.97, 0.70, 0.66, 0.59, 0.52, 0.50, and of the current character alone 0.91, 0.30,
0.22, 0.14, 0.12, 0.13 (Figure 37c). Head units (best rank under 32, $n=668$) reach 0.80 and 0.42 where
tail units (best rank 512 or worse, $n=1{,}623$) reach 0.51 and 0.12 ($p=2\times10^{-67}$ and
$2\times10^{-97}$, Mann–Whitney over distinct units). The step down happens immediately after the top 8
and then flattens, with no second mode anywhere: the tail is made of the same kind of thing as the head,
weaker and harder to describe from short character context. This is an association measured at one
checkpoint between a unit's rank and how well text predicts it; it names no mechanism.

Redundancy and weakness are properties of the finished network, and the obvious next question is
whether training built them. Repeating the decomposition at five checkpoints, and asking separately
which units occupy the head at each one, answers it — and both answers came out against the guess that
motivated the run (Figure 38).
...
whose membership it keeps rewriting. Same run, same 150 pairs, interpolation after block 0, checkpoints
at steps 831, 2,038, 5,000, 12,500 and 30,000; panels (a)–(c) use the 94 pairs that clear the 0.10 gap
filter at every checkpoint. **(a)** Percentage of the trained-to-untrained width gap removed (y) against
training step (x, log scale): circles (solid) = all 3,840 units linearized at once, squares (dashed) =
the sum of the six bands each measured alone. **(b)** Redundancy ratio $\Lambda$, the ratio of those two
curves per pair, median over the 94 pairs (y) against training step (x, log scale); the gray dashed line
at 1.0 marks exact additivity. **(c)** Percentage of the gap removed by each band alone (y) against
training step (x, log scale), one series per band with distinct marker and line style; the bottom band
(ranks 2,048–3,840) sits at zero throughout and is omitted. **(d)** Percentage of a checkpoint's
per-pair top-$k$ units that are also in that pair's step-30,000 top-$k$ (y, median over the 150 pairs)
against training step (x, log scale): circles (solid) $k=8$, squares (dashed) $k=32$; chance is 0.2% and
0.8%.

**Redundancy is there from the start.** The ratio $\Lambda$ reads 1.21, 1.01, 1.08, 1.21, 1.18 across the
five checkpoints of this run — a dip at step 2,038 and no net change from end to end (Figure 38b); the
second training run repeats the level but drifts up a little, which is taken up with Figure 43 below.
Overlap between bands is
there at step 831, when the median width is still 0.52 on these pairs and the network has barely begun
to sharpen. What does grow is how much of the bend these units account for at all: 46.2% → 81.0% of the
gap for all units at once (Figure 38a), concentrated at the top of the ranking, where the top-8 band
climbs 7.2% → 23.9% while the 512–2,048 band moves 7.8% → 10.0% and the bottom band stays at zero
throughout (Figure 38c).

**The head is re-selected, not amplified.** The natural reading of a strengthening head is that the same
eight units grow into the job. They do not. The median overlap between a checkpoint's per-pair top-8 and
the same pair's step-30,000 top-8 is 0, 2, 4, 6, 8 units across the five checkpoints, and 6, 10, 16, 23,
32 for the top-32 sets, against chance overlaps of 0.02 and 0.27 units (Figure 38d). At step 831 the
top eight units already remove 7.2% of the gap and **none of them** is a unit the finished network ranks
in its own top eight. Overlap between consecutive checkpoints (3, 3, 5, 6 of 8) shows the turnover
continuing late in training. The units that carry the plateau boundary at the end are largely a
different set from the ones that carried it early.

**They are promoted from just below the head, not recruited from nowhere.** Zero overlap says the final
head units were not in the head early; their **rank** says where they were, and it rules out the reading
that they arrive from the anonymous middle of the ranking (Figure 39). At step 831 a pair's eventual
top-8 units already sit at median rank **113.5** of 3,840, against 1,919.5 for a unit drawn at random,
and **51.8%** of them are already inside that checkpoint's top 128 (75.5% inside the top 512). From
there they climb smoothly — median rank 113.5 → 31 → 7 → 4 → 3.5 across the five checkpoints, with
**75%** of that climb completed by step 2,038 — while entry into the top 8 itself is the last thing to
happen (9.8%, 23.3%, 51.6%, 72.0%, 100% of final head units are inside the running top 8). The displaced
early leaders do not disappear either: the step-831 top-8 units drift to median rank 3.5 → 15 → 42 → 74
→ 100.5, still an order of magnitude above chance at the end. So what training rewrites is the ordering
inside a broad candidate pool that is recognizable from early on, which is the same picture as the
shared pool of 668 units that the per-pair top-32 sets are drawn from.
...
nowhere. Same run, 150 pairs, interpolation after block 0, no ablations — one recording pass per pair per
checkpoint. **(a)** Importance rank (y, log scale; 0 = most important, 3,839 = least) against training
step (x, log scale). Circles (solid) = each pair's step-30,000 top-8 units, read at every checkpoint;
squares (dashed) = each pair's step-831 top-8 units, read at every checkpoint. Bands are the
interquartile range over the 1,200 pair–unit trajectories; the gray dotted line at 1,919.5 is the rank a
uniformly drawn unit has in expectation. **(b)** Percentage of each pair's step-30,000 top-8 units that
are already ranked inside the checkpoint's top 8, 32, 128 and 512 (y) against training step (x, log
scale); each threshold has its own marker and line style.

**In the finished network, the describable head units are the early-selected ones.** Knowing when a unit joins the head
does not say what it computes, and the fitted character-window probe answers that for every unit at
once. Read without conditioning, the comparison looks like a story about training selecting describable
units: the 181 promoted units beat the 141 demoted ones with median held-out $R^2$ **0.72 against 0.41**
from the current character alone (effect size 0.58, $p=0.013$) and **0.92 against 0.77** from the full
window (0.56, $p=0.079$), both groups far above the all-unit medians of 0.15 and 0.59 (Figure 40a).
That reading does not survive the confound built into the labels. Promoted units sit at best rank 0–7 by
construction and demoted units at 8 or worse, and rank alone already tracks describability (Figure 37c).
Comparing groups **inside** a band of best rank reverses the direction. Within the finished head (ranks
0–7), the 75 units that also held the head at step 831 are more describable than the 181 units training
promoted into it: $R^2$ **0.97 against 0.72** from the current character (effect size 0.69,
$p=3\times10^{-6}$) and **0.99 against 0.92** from the full window (0.67, $p=3\times10^{-5}$)
(Figure 40b). One band down (ranks 8–31), the 66 demoted units are far more describable than the 346
units at the same rank that were never in either head: **0.94 against 0.23** (effect size 0.79,
$p=9\times10^{-14}$) and **0.98 against 0.66** (0.75, $p=1\times10^{-10}$) (Figure 40c). At the final
checkpoint, then, early-head membership rather than present rank is what describability tracks: the
units this network leaned on after 831 steps are close to what a single character predicts, and they
stay so after being displaced, which is why they are still recognizable one band below the head.
...
the same 150 pairs. Each panel is a cumulative distribution: x = held-out $R^2$ (0 = the probe does no
better than the unit's mean, 1 = the unit's corpus response is fully described), y = the fraction of that
group's units at or below that $R^2$, so a curve further **right** is the more describable group. Top row
(a1, b1, c1): $R^2$ from the current character alone; bottom row (a2, b2, c2): $R^2$ from the full
8-character window with the interaction term. **(a)** promoted (solid) vs demoted (dashed) with all 3,840
units as a gray dotted reference. **(b)** among units of best rank 0–7: stable (solid) vs promoted
(dash-dot). **(c)** among units of best rank 8–31: demoted (solid) vs never-head (dash-dot). Panel titles
give the common-language effect size and the Mann–Whitney $p$.

**Describability moves with head membership, not with the units.** Everything above is measured on one
model, so it cannot distinguish "training keeps a set of describable units and adds harder ones" from
"whatever currently holds the head is the describable thing". Refitting the same probe at the four
earlier checkpoints separates them, and the second reading is what the numbers support (Figure 41). Two
facts do the work. The network as a whole becomes *less* readable from characters as it trains: the median unit's
$R^2$ from the current character falls **0.39 → 0.28 → 0.24 → 0.17 → 0.15** across the five checkpoints
(0.64 → 0.59 for the full window at the ends), which is the background every group has to be read
against. Against that background the promoted units move the other way — median $R^2$
**0.61 → 0.64 → 0.67 → 0.73 → 0.72** from the current character, a per-unit median change of **+0.05**
while the never-head units lose **0.17** — and the demoted units fall faster than the background,
**0.92 → 0.83 → 0.60 → 0.41 → 0.41** (median change **−0.25**), most of that fall happening between
steps 2,038 and 12,500 — the same span over which their replacements take the head, the top-8 overlap
with the final head climbing from 2 to 6 of 8 (Figure 38d). The stable units stay at the ceiling
throughout (0.96, 0.98, 0.98, 0.98, 0.97). So describability is not a fixed property of particular units
that training selects for or against; a unit is character-readable while it is in the head, and the two
move together in both directions. How far ahead this is visible is a separate question, and the answer is: a little.
Inside the step-831 head, the 75 units that would keep it are barely separable from the 141 that would
lose it (median $R^2$ 0.96 against 0.92, effect size 0.59, $p=0.035$), and one band below (step-831 best
rank 8–31) the 68 units that would be promoted are moderately more describable than the 337 that never
hold the head (0.80 against 0.64, effect size 0.63, $p=7\times10^{-4}$).
...
block-1..4 units and the same probe as Figure 40, refitted at each of the five checkpoints; groups are
the same four roles. **(a)** Cumulative distribution at step 831: x = held-out $R^2$ from the current
character alone, y = the fraction of that group's units at or below it, so a curve further **right** is
the more describable group; stable (solid), demoted (dashed), promoted (dash-dot), never-head (dotted).
...
**A second training run separates the recipe from the initialization.** Every number above comes from
one training run, and a developmental story is exactly the kind of claim a single initialization can
manufacture. Retraining the identical recipe with model seed 2024 (23 minutes; final validation accuracy
0.551 against the reference run's 0.550) and repeating all three measurements on the new run's
checkpoints shows which of them are properties of the recipe (Figure 42). Three reproduce closely. The
paths sharpen on the same schedule: median transition width **0.641 → 0.472 → 0.376 → 0.345 → 0.338**
across the five checkpoints, against 0.653 → 0.484 → 0.368 → 0.365 → 0.351 in the reference run. The
head is rewritten the same way: the median overlap between a pair's top-8 at a checkpoint and its own
step-30,000 top-8 runs **1, 2, 4, 6, 8** units (reference 0, 2, 4, 6, 8) and **7, 12, 15, 22, 32** for
the top-32 sets (reference 6, 10, 16, 23, 32), with the same continuing turnover between consecutive
checkpoints. And the final head units are again promoted from just below the head: median rank **126**
of 3,840 at step 831 against the chance rank 1,919.5 (reference 113.5), **50.2%** of them already inside
that checkpoint's top 128 (reference 51.8%), climbing 126 → 20 → 7 → 4 → 3.5 with **86.5%** of the climb
done by step 2,038 (reference 75%), while the displaced early leaders settle at median rank **98**
(reference 100.5). The structural developmental picture — turnover, promotion from a recognizable early
pool, and the sharpening it accompanies — is a property of this training recipe.

**One half of the describability result is seed-specific, and it is the forward-looking half.** The parts
of Figure 41 that concern units *losing* the head reproduce: the whole network again becomes less
character-readable (median unit $R^2$ from the current character **0.40 → 0.28 → 0.27 → 0.19 → 0.17**,
reference 0.39 → 0.15), the 153 demoted units again fall faster than that background
(**0.92 → 0.89 → 0.68 → 0.44 → 0.42**, per-unit median change **−0.27**, reference −0.25) and the 86
stable units again sit at the ceiling throughout (0.96–0.99). At the final checkpoint the two band-matched
contrasts also hold, with larger effects than in the reference run: inside the finished head the stable
units beat the 184 promoted ones **0.98 against 0.47** (effect size 0.77, $p=6\times10^{-13}$; reference
0.97 against 0.72, 0.69), and one band down the demoted units beat the same-rank never-head units
**0.81 against 0.22** (0.78, $p=5\times10^{-15}$; reference 0.94 against 0.23, 0.79). What does not carry
over is the claim that training makes the units it promotes more describable in absolute terms. In the
second run the promoted group's median $R^2$ from the current character *falls* slightly,
**0.60 → 0.56 → 0.56 → 0.50 → 0.47** (per-unit median change **−0.02**, against **+0.05** in the
reference run); measured against the falling background it still rises — from 1.5× to 2.7× the all-unit
median, where the reference run went 1.6× to 4.7× — and on the full-window probe it still gains a little
in absolute terms (+0.02, reference +0.09). The forward-looking contrast at step 831 fails outright: the
74 units one band below the early head that will be promoted are indistinguishable from the 322 that
never hold the head (**0.69 against 0.68**, effect size 0.53, $p=0.4$), where the reference run found
0.80 against 0.64 ($p=7\times10^{-4}$). Inside the early head the mirror contrast is *stronger* in the
second run (stable 0.96 against demoted 0.92, effect size 0.64, $p=4\times10^{-4}$; reference 0.59,
$p=0.035$). Read together, what two runs support is the backward-looking statement — a unit is
character-readable while it holds the head, and readability drains away with membership — while an early
checkpoint carries essentially no information about which units below the head will be promoted.
...
units' rise in describability does not. Reference recipe trained twice (model seed 1337 and model seed
2024, everything else identical), same 150 pairs, same context, same block-0 interpolation, same five
checkpoints. In every panel x is the training step of the checkpoint (log scale), seed 1337 is drawn
with filled markers on solid lines and seed 2024 with open markers on dashed lines, and each series
within a panel has its own marker shape as well as its own colour. **(a)** y = the fraction of a pair's step-30,000 top-$k$ units already held at that
checkpoint (median over pairs), for $k=8$ and $k=32$: the head turns over at the same rate in both runs.
**(b)** y = median importance rank of 3,840 (log scale; 0 = most important), for each pair's step-30,000
top-8 units read at every checkpoint and for its step-831 top-8 units read at every checkpoint; the gray
dotted line at 1,919.5 is the rank a uniformly drawn unit has in expectation. **(c)** y = median
held-out probe $R^2$ from the current character alone, for the promoted group, the demoted group and all
3,840 units. The demoted fall and the whole-network decline coincide across runs; the promoted curves
diverge, rising in seed 1337 and drifting down in seed 2024.

**The band decomposition reproduces at the second seed too.** The redundancy, the reach of the ranking
and the describability gradient of Figures 37 and 38 were also one-run measurements, and repeating them
on the seed-2024 checkpoints costs only forward passes (Figure 43). All three come back. At step 30,000
the six bands remove **25.6, 26.1, 21.6, 18.9, 8.1** and **−0.1%** of the width gap on their own,
against 27.9, 25.9, 24.1, 20.4, 10.4 and −0.1% in the reference run — every band within 2.5 points — and
they sum to **105.6%** against an all-units ceiling of 84.0%, a redundancy ratio $\Lambda=1.28$ per pair
(paired $p=6\times10^{-23}$, 90.0% of the 150 pairs above 1; reference 1.29 with 86.7%). Per-unit worth falls the
same way, from 32.0% of the gap per thousand units in the top band to 0.053% in the 512–2,048 band, a
610-fold decline (reference 34.8% to 0.067%, 520-fold), and every band above rank 2,048 again beats a
same-size random draw from its own region of the ranking (25.6 against 0.05, 26.1 against 0.19, 21.6
against 0.74, 18.9 against 2.3, and 8.1 against 2.9%, each $p\le8\times10^{-26}$ and 95–100% of pairs),
while the bottom 1,792 units again bend the path not at all. Reading the refitted character-window probe
by best-rank band gives the same smooth decline over the six bands (full description 0.96, 0.64, 0.64,
0.59, 0.50, 0.47; current character alone 0.91, 0.27, 0.22, 0.15, 0.13, 0.15; reference 0.97, 0.70, 0.66,
0.59, 0.52, 0.50 and 0.91, 0.30, 0.22, 0.14, 0.12, 0.13), with head units — best rank under 32, 707 of
them — at 0.72 and 0.34 against the 1,664 tail units at 0.49 and 0.14 ($p=6\times10^{-72}$ and
$4\times10^{-66}$; reference 0.80 and 0.42 against 0.51 and 0.12).

**What two runs do not settle is whether redundancy grows.** In the reference run $\Lambda$ ends where
it began — 1.21 at step 831 against 1.18 at step 30,000, per-pair median change −0.01, $p=0.9$ — which
is the "flat, no trend" reading of Figure 38b. The second run starts lower and drifts up: 1.10, 1.12,
1.05, 1.16, 1.20 across the same checkpoints, a per-pair median change of **+0.10** ($p=2\times10^{-5}$,
68% of pairs up). Both runs agree on the part the claim rests on. At the earliest checkpoint, where the
median width is still 0.52 and 0.56 and the paths have barely begun to sharpen, the bands already
overlap by 10–20%, so the redundancy is there as soon as there is a bend to share rather than being
assembled late in training; and in both runs the drift over the whole of training is small beside the
standing distance from additivity. Which way that drift goes is a two-point question that two runs
cannot answer.
...
the direction of the small drift in redundancy over training. Reference recipe trained twice (model
seed 1337 and model seed 2024, everything else identical), same 150 pairs, same context, same block-0
interpolation, same six bands. Seed 1337 is drawn with filled markers on solid lines and seed 2024 with
open markers on dashed lines. **(a)** Redundancy ratio $\Lambda$ — the sum of the six band-alone effects
divided by the all-units effect, per pair — median over the pairs usable at every checkpoint (94 in seed
1337, 103 in seed 2024) on the y axis, against training step on the x axis (log scale); the gray dashed
line at 1.0 marks exact additivity. **(b)** Percentage of the trained-to-untrained width gap removed at
step 30,000 (y) against importance-rank band (x; band size in the tick label): circles = the band
chord-linearized alone, squares = a same-size random draw from the same region of the ranking.
**(c)** Median held-out $R^2$ of the character-window probe, refitted on each run's own step-30,000
...
**What the leftover response is not: position in the line, position in the word, word identity, or the
text before the window.** Widening the character window leaves a real gap: at step 30,000 the demoted
units sit at median $R^2$ 0.78 under all40, so about a fifth of their corpus response is not a linear
function of the identity of any character in a 40-character neighbourhood. Naming that fifth would say
what these units came to compute, and two candidates are cheap to test — text past the probe's window,
and structure that is not character identity at all. Adding the three families defined in Methods
(struct, lex, bag) to the same fits answers both (Figure 45).

Neither candidate accounts for the gap. Adding all three to all40 raises the demoted units from 0.780 to
**0.815** at step 30,000 — a median gain of **+0.020** per unit, and every one of the 141 units gains
something ($p=7\times10^{-25}$) — but that closes only **9.2%** of what all40 was missing (residual
0.220 → 0.185). The remaining nine tenths stay unexplained. The gain is also not particular to these
units: it closes 10.1% of the stable units' (much smaller) residual, 9.2% of the promoted units' and
11.6% of the never-head units', so between 9% and 12% of whatever each group has left over, whichever
group it is. A probe extension that had found what the demoted units switched to would show up as a gain
concentrated on them; this one behaves like a small uniform improvement to the probe (Figure 45b).

The developmental fall is untouched to three decimals. Between step 831 and step 30,000 the demoted
units' median per-unit change is **−0.0637** under all40 and **−0.0637** under union
($n=141$, $p=5\times10^{-13}$); network-wide the fall grows slightly rather than shrinking (−0.045 →
−0.058, $p=5\times10^{-69}$). Nine feature families have now been fitted at both ends of training — five
in Figure 44 and four here — and the size of the fall has not moved since the eight-character window.
Whether that fall belongs to the units or to the linear form of the probe is the question the next
subsection settles.

Taken on their own the new families are weak exactly where the residual is (Figure 45c). At step 30,000
the structural family reaches median $R^2$ 0.053 across the network and 0.047 on the demoted units; the
bag family reaches **0.0002**, i.e. nothing, so the earlier text inside the window carries no separable
signal at all. The lexical family looks stronger (0.35 network-wide, 0.45 on the demoted units), but its
features are built from the same characters the past families already use — a word or trigram identity
is a conjunction of nearby characters — so its $R^2$ is not evidence of a separate lexical variable, and
it declines over training like everything else (median −0.12 on the demoted units). One further sign
that the probe as a whole is losing ground: the same union closes 18.2% of all40's network-wide residual
at step 831 and only 11.5% at step 30,000.
...
step 30,000: x = gain, y = the fraction of units at or below, one line per role with its own marker; the
x axis is clipped at 0.30, above which 1.6% of units lie. Stable units (circles) sit almost on zero
because they have almost nothing left to explain; never-head units (diamonds) gain most because they
have most left. **(c)** Median held-out $R^2$ at step 30,000 (y) for each family fitted on its own (x),
for the demoted units (circles/blue) and the stable units (triangles/reddish purple); the stem under
each marker is drawn to zero for readability.

**A large part of the leftover response is a nonlinear function of the characters the probe already
sees.** Nine
feature families have now failed to name the fifth of a demoted unit that all40 misses, but every one of
them was fitted by ridge, so every one of them is linear in its design. A unit that fires for a
*combination* of nearby characters — an apostrophe inside a capitalised word, a comma that follows a
short word — would be invisible to all nine no matter how wide the window, because the missing feature
is a shape of the read-out and not a column of the design. Keeping all40's design and replacing ridge
with a one-hidden-layer network (Methods) separates those two possibilities, and its control — the same
network with the nonlinearity deleted — keeps everything else fixed (Figure 46).

The control lands where it should: fitted by Adam on the same rows, the linear network reproduces the
published ridge fit unit by unit (median difference **−0.002** at both checkpoints, $r=1.00$ and
$0.999$, $n=797$), so anything the hidden layer adds is the nonlinearity and not the change of
optimiser. What it adds is large. At step 30,000 the demoted units rise from median held-out $R^2$
**0.778** under the linear read-out to **0.921** under the nonlinear one — a median gain of **+0.044**
per unit with 70% of the 141 units improving ($p=5\times10^{-15}$) — and the gain is largest exactly
where the most was left over: the never-head units go from 0.599 to **0.861** (median gain +0.179, 94%
of 400 units up), the promoted units from 0.918 to 0.935, and the stable units, which had 1.3% of their
response unexplained to begin with, gain nothing at all (−0.0005, $p=0.53$). Read per unit — for each
unit, the share of its own unexplained variance that the hidden layer removes — the median unit's residual
shrinks by 53% among the never-head units, 26% among the demoted, 15% among the promoted and −5% among the
stable. The extra feature families of Figure 45b closed a similar share for every role whatever it had
left; this gain instead grows with how much a group has left to explain.

The consequence for the developmental story is the part worth stating plainly. Under the linear read-out
the demoted units lose **−0.064** of $R^2$ between step 831 and step 30,000; under the nonlinear one they
lose **−0.026** ($n=141$, $p=4\times10^{-7}$), so a nonlinear read-out of the same 40 characters
accounts for about 60% of the decline. Across all 797 probed units the decline does not merely shrink but
reverses (**−0.011** under ridge, **+0.019** under the nonlinear probe, $p=2\times10^{-17}$): the median
unit is *better* described at step 30,000 than at step 831 once the probe may combine characters.
The network-wide half of "units become harder to describe over training" is therefore a statement about
linear read-outs, not about the units — the material stays inside the same 40 characters and stops being
a weighted sum of them. The demoted units are the exception at this probe size: 40% of their fall
survives an estimator that reverses the network-wide fall, and that surviving part, together with the 8%
of a demoted unit's response still unexplained at step 30,000, is what nine feature families and this
read-out fail to name. Whether it is a fact about the units or about the size of this particular network
is the question Figure 47 answers.

One diagnostic from the fitting itself points the same way. Under one stopping rule — training halts
after 20 epochs with no validation improvement — the nonlinear probe reached its best score after 66
epochs at step 831 and after 572 at step 30,000, nearly nine times as long, which is a second and
independent sign that what these units compute becomes a harder function to fit rather than a smaller
one.
...
fall in place. Reference run, 797 block-1..4 units (all 75 stable, 141 demoted and 181 promoted units and a
fixed random 400 never-head units), same rows and same 80/10/10 split as Figures 44 and 45. **(a)** y =
median held-out $R^2$; x = the probe (ridge = the published all40 fit; linear = the same design fitted
as a network with the nonlinearity deleted; one hidden layer = the nonlinear read-out). Each vertical
bar joins a group's step-831 value (filled marker) to its step-30,000 value (open marker); circles/blue
= the 141 demoted units, squares/vermillion = all 797 probed units. **(b)** Cumulative distribution over
units of the per-unit $R^2$ gain from the hidden layer (nonlinear minus linear) at step 30,000: x =
gain, y = the fraction of units at or below, one line per role with its own marker, x clipped at 0.60
(0.8% of units lie above). The never-head curve (diamonds) lies far to the right of the stable curve
(circles), which sits on zero.
**(c)** y = median per-unit change in $R^2$ from step 831 to step 30,000 (negative = the units became
harder to describe); x = the unit's role, one marker per probe. The ridge and linear markers coincide;
the nonlinear probe lifts every role, and only the demoted units stay clearly below zero.

**A larger probe of the same kind keeps closing the gap, and the demoted units' fall goes with it.**
What the width-768 network cannot fit might be what that particular network is too small to fit, and
until that is checked, nothing about the units follows from what it leaves behind. Holding the design,
the rows, the split, the optimiser and the stopping rule fixed and varying only the probe's capacity —
width 1,536 with one hidden layer, and width 768 with two hidden layers (Methods) — settles the question
(Figure 47).

Capacity is still the binding constraint at the end of training. At step 30,000 the median held-out
$R^2$ over all 797 probed units rises **0.726** (linear) → **0.889** (width 768, one hidden layer) →
**0.929** (width 1,536) → **0.940** (width 768, two hidden layers), and on the demoted units
**0.778** → **0.921** → **0.949** → **0.952**: the share of a demoted unit's response that no probe
explains falls from 7.9% under the previously reported nonlinear probe to 5.1% and 4.8%, with median
per-unit gains of **+0.016** (96% of the 141 units up, $p=5\times10^{-18}$) and **+0.015** (71% up,
$p=5\times10^{-12}$). Nothing has flattened; each step in capacity still removes roughly a quarter of
what the previous probe left.

Early in training the same extra capacity buys nothing. At step 831 the three nonlinear probes agree to
within 0.004 on every role (all-unit medians 0.878 / 0.881 / 0.880), and under the same 20-epoch
patience rule they converge in 28–41 epochs there against 336–485 epochs at step 30,000. Both
observations say the same thing: what these units compute late in training is a harder function to fit
than what they compute early — and it is a function of the same 40 characters.

The consequence for the developmental claim is that its last surviving piece does not survive. The
demoted units' median per-unit change in $R^2$ from step 831 to step 30,000 is **−0.064** under the
linear read-out, **−0.026** under the width-768 nonlinear one, and **−0.005** ($p=0.015$) and
**−0.004** ($p=0.13$, $n=141$) under the two larger probes, while the other three roles rise further the
more capacity the probe has (never-head +0.094 and +0.103, promoted +0.067 and +0.077, stable +0.006 and
+0.005). "Units that lose head membership become harder to describe over training" therefore holds under
every linear probe in this series and under the smallest nonlinear one, and stops holding once the
read-out is large enough: measured by a sufficiently large nonlinear function of the same 40 characters,
a demoted unit is about as describable at step 30,000 as it was at step 831.

Two things about this result are worth keeping separate. The decline is now attributed to the probe
rather than to the units — that is a statement the experiment supports, because capacity is the only
thing that changed between the four fits. The *level* is not settled: 4.8% of a demoted unit's response
is still unexplained at step 30,000 and the capacity curve is still climbing, so that number is a bound
set by the largest probe we fitted, not a property of the units. What survives about development is a
statement about the form of the description rather than its quality — at step 30,000 the same 40
characters describe these units only through a read-out that is nonlinear, larger, and eight to
seventeen times slower to fit than the one that suffices at step 831.

![two panels: median held-out R-squared at step 30,000 against probe capacity for the four unit roles, and the median per-unit change in R-squared between checkpoints against probe capacity for the same roles](plots/neuron_probe_capacity.png)

**Figure 47.** More probe capacity keeps raising the late-checkpoint fit and removes what was left of the
demoted units' describability decline. Reference run, the same 797 block-1..4 units, rows and 80/10/10
split as Figure 46; the two left-hand probes are Figure 46's fits re-used, the two right-hand ones are
new. **(a)** y = median held-out $R^2$ at step 30,000; x = the probe's capacity in increasing order
(linear = width 768 with the nonlinearity deleted; then width 768 with one hidden layer, width 1,536 with
one hidden layer, and width 768 with two hidden layers). One line per unit role with its own marker:
stable (circles), demoted (squares), promoted (triangles), never-head (diamonds). Every role except the
stable units, which start at the ceiling, keeps rising to the right. **(b)** y = median per-unit change
in $R^2$ from step 831 to step 30,000 (negative = the units became harder to describe over training); x =
the same capacity axis, same four roles. The demoted line (squares) is the only one below zero and it
rises to −0.005 and −0.004 at the two largest probes.

**Why this matters.** Four practical consequences follow from Figures 37 to 47. First, the concentrated
...
using such an intervention to argue that a behaviour has been ablated needs the band-alone measurement,
not the prefix curve. Second, the importance ranking is worth trusting to about rank 2,048 and worth
nothing below that, which sets where a unit-selection method should stop looking. Third, a head that is
rewritten as training proceeds means unit identity found at one checkpoint transfers poorly to another
in this run, even though the *count* of units needed stays similar — an early-checkpoint interpretation
would have named the wrong eight units. Figure 39 bounds how wrong: half of the finished head is
already inside the early top 128, so an early checkpoint identifies the right neighbourhood of the
ranking while getting its order wrong, and a study that needs unit identities should either use the
checkpoint it is reasoning about or widen $k$ until the pool is covered. Fourth, and this is the part
that bears on how such a head gets described, a simple description of it is checkpoint-specific in the
same way its membership is. At the final checkpoint the units a short character window explains well are
the early-selected ones at every rank we can condition on (Figure 40b, c), and both training runs show
...
checkpoint attached to it — and, on the evidence of Figures 44, 46 and 47, the probe's feature family,
the form of its read-out and its size attached to it as well, since the same decline is four times
smaller once the probe may use seven characters of context, reverses sign network-wide once the probe may
combine those characters nonlinearly, and disappears on the demoted units too once that nonlinear probe
is made larger. An "interpretability fraction" measured with a linear probe on a one-hot design is as
much a property of that probe as of the network. The second run also
marks the limit of that reading: whether the units training *promotes*
...
The band-alone effect of a set is a property of this chord intervention. The character-profile comparison
in Figure 37c is a rank-describability association at the final checkpoint and carries no causal claim.
Figures 42 and 43 give a second seed to the turnover, promotion and describability trajectories, to the
band-alone and all-units effects at every checkpoint, to the within-region control and to the
rank-describability gradient; the marginal column $\Delta_b$ inside the nested prefix, the naive random
draw from all 3,840 units and the greedy re-selection remain reference-run measurements. Two runs show
whether a trend has the same sign and rough size, and no more than that — the spread across
initializations is not estimated from two points, and where the two disagree in sign, as the drift in
$\Lambda$ does, neither run settles the question. Figures 40 to 42 also stay associational: they relate a
unit's role to how well one probe family describes it, and nothing in them shows that a unit is promoted
...
all. "Describable" is always relative to a probe, and three things about the probe now matter. Figures 44
and 45 remove the nearest alternative feature sets — 32 characters of history, eight characters of
lookahead, position in the line and in the word, word and trigram identity, and the character composition
of the earlier window — and the decline survives every one of them. Figures 46 and 47 then vary the
read-out instead of the features, and that is the change that matters: a large part of what the linear
families miss is a nonlinear function of the characters they already hold, the network-wide decline goes
with the linear form of the read-out, and the demoted units' own fall goes with the size of the nonlinear
one. What survives the whole series is the *level* — 4.8% of a demoted unit's response at step 30,000 is
unexplained by the largest probe fitted here — and, since the capacity curve of Figure 47a is still
climbing at that probe, even that is a bound on what a still larger read-out would leave rather than a
property of the units. The capacity series is two architectures at one learning rate under one stopping
rule; it shows that the decline does not survive more capacity, and it does not identify how much
capacity is enough. All four figures are two-checkpoint measurements on the reference run; the second
seed was not refitted with any of the wider families or with any of the nonlinear probes. Free checks: the nested-prefix
...
$R^2$ (0.5502 against 0.5502; Jaccard 1.00; $r=0.95$ and $0.99$), and the larger design behind Figure 45
reproduces Figure 44's char1, past8 and all40 fits unit by unit (largest deviation over 3,840 units: 0,
0 and $5\times10^{-13}$), and its structural and bag features were checked against a plain-Python
rebuild on random corpus positions before the run, and the linear control behind Figure 46 reproduces the
published ridge all40 fit over the 797 units it shares with it (median difference −0.002, $r=1.00$ at
step 831 and $0.999$ at step 30,000).
...
`results/neuron_probe_nonlinear_summary.json`,
`results/neuron_probe_capacity_summary.json`; code:
...
The structure is visible pair by pair, with no averaging involved (Figure 48).

![exploratory 40-pair raw curves](plots/pair_curves_logits.png)

**Figure 48.** *(Exploratory.)* Raw relative distance $d(t)$ (y) vs interpolation position $t$ (x) in
...
downstream stack, not present in the interpolated activation itself (Figure 49).

![exploratory layerwise emergence](plots/layerwise_emergence.png)

**Figure 49.** *(Exploratory.)* Layerwise emergence for four fixed representative pairs (IDs 0–3,
...
reaching the straight-line reference 0.8 when only one block remains (Figure 50).

![exploratory interpolation-block comparison](plots/interpolation_layer_comparison.png)

**Figure 50.** *(Exploratory.)* Left: median final-logit $d(t)$ (y) vs interpolation position $t$ (x)
...
plateaus). So the prediction flip stays an accurate marker of the geometry and is ruled out as its
...
unit's write norm costs about a point, and the write norm alone selects 0.3%. Nor is the residue a
large joint effect: rebuilding the set in eight rounds, re-scoring every unit against the bend still
left after the earlier picks are straightened, gains 3.4 points at 128 units (68.4% → **71.8%**) and
nothing at 32, so unit interactions close about a fifth of the distance to the all-units ceiling and do
so only in the interchangeable tail of the ranking. No geometric rule over
...
7. **The next-character decision neither labels the basins nor explains them.** It fails as a label
   ...
   *which* pairs are sharp (partial $\rho = -0.59$) but is excluded as the mechanism
   ...
   (Figure 30), and identifiable in advance as the character detectors tuned to the pair's own
   endpoints (Figure 31, AUROC 0.847; linearizing 32 units chosen that way, blind to the assay,
   ...
   than character-pure (Figure 33), which conditioning the profile on the preceding character does not
   ...
   removes 56.5% of the width gap (Figure 34) — more than the ranking fitted to the curve, with the
   controls attributing most of that to keeping each unit's scale rather than to the added context.
   That text-only rule is at the ceiling of its family (Figure 35): an oracle reading the network's own
   endpoint activations ties it, and weighting by the units' write norms — which span a factor of only
   1.71 — slightly hurts. The joint effect of a set of units, which that ceiling left open, is now
   measured and small (Figure 36): sequential re-selection over eight rounds gains 3.4 points at 128
   units and nothing at 32, so what stays uncharacterised is not which units are chosen but why so much
   of the sharpness needs hundreds of them.
   The freezing
