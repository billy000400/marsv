...
for ax, st in zip(axes, steps):
    for pi, p in enumerate(pairs):
        ...
        ax.plot(ts, d, lw=2, color=f"C{pi}", label=p)
...
for name, color in [("train", "C0"), ("test", "C1"), ("random", "C2")]:
    ...
    a1.plot(x9, m, color=color, lw=2, label=f"LC {name}")
    a1.fill_between(x9, m - ci, m + ci, color=color, alpha=0.25)
a2.plot(x9, [r["clean_acc"] for r in fig9], "k-", lw=2, label="test acc (clean)")
a2.plot(x9, [r["adv_acc"] for r in fig9], "r--", lw=2, label="adv acc (PGD $\\epsilon$=0.03)")
...
for pi, p in enumerate(pairs):
    ...
    a3.plot(xm, ws, "o-", color=f"C{pi}", lw=2, label=f"w ({p})")
a3.axhline(0.8, color="gray", ls="--", lw=1, label="diagonal (no plateau) w=0.8")
a3.axhline(0.25, color="red", ls=":", lw=1, label="plateau rule w=0.25")
a3.set_xscale("log")
