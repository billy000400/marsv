...
Exp 26/27/28/29 put 5-seed confidence intervals on four models — GPT-2 small
(83.3 ± 2.0%), GPT-2 medium (88.3 ± 2.2%), Pythia-410m (80.8 ± 1.6%), Qwen3-1.7B
(94.8 ± 1.6%) @α=8. The ONE remaining headline model still reported from a single
seed-0 run is **GPT-2 large (Exp 19, block 18/36)** — 84% @α=8 / 95% @α=4. This is
PLAN Next-step option (i)'s final item: close the last single-seed point so every
model the study leans on carries an error bar.

This script re-runs the EXACT Exp-19 GPT-2-large pipeline (same DiffMean sentiment
vector at block 18, same 400-doc Gaussian fit, 300-doc train / held-out 100-doc eval,
same 6.03M corrector at d=1280, same recipe, matched projection, same VRAM-safe batch
sizes) at N seeds and reports mean ± std of the fluency recovery
...
seed 0 reproduces Exp 19 to the digit — a built-in reproducibility check.

Reuses the Exp-19 module (retarget/diffmean, VRAM batch sizes) and the Exp-3 module
(Corrector/make_hat/train_corrector/eval helpers) verbatim, overriding exp03.SEED.
Outputs: results/30_seed_robustness_large.json + plots/30_seed_robustness_large.png.
"""
...
exp19 = _load("exp19", "19_gpt2_large.py")   # GPT-2 large retarget + diffmean + VRAM batches
exp03 = _load("exp03", "03_learned_corrector.py")

MODEL_NAME = exp19.MODEL_NAME    # gpt2-large
LAYER = exp19.LAYER              # 18 of 36
D_MODEL = exp19.D_MODEL          # 1280
FIT_BATCH, TRAIN_BATCH, EVAL_BATCH = exp19.FIT_BATCH, exp19.TRAIN_BATCH, exp19.EVAL_BATCH


def main():
    exp19.retarget()                 # install GPT-2 large into common's cache
    exp03.LAYER = LAYER
    ...
    # --- clean Gaussian fit + eval set — identical to Exp 19, deterministic across seeds ---
    ...
    train_texts = all_texts[500:800]     # 300 docs (same split as Exp 19)
    ...
    for a in ALPHAS:
        ...
        dlm_raw.append(exp03.lm_loss_fn(eval_texts, LAYER, f_raw, seq_len=128, batch=EVAL_BATCH) - clean_loss)
    ...
    for seed in SEEDS:
        ...
        for a, draw in zip(ALPHAS, dlm_raw):
            ...
            dl = exp03.lm_loss_fn(eval_texts, LAYER, f_learn, seq_len=128, batch=EVAL_BATCH) - clean_loss
    ...
    with open(os.path.join(RESULTS, "30_seed_robustness_large.json"), "w") as f:
    ...
    ax[1].errorbar(ALPHAS, rec_m, yerr=rec_s, fmt="o-", color="#2980b9", capsize=4,
                   label=f"large: mean ± std ({len(SEEDS)} seeds)")
    for s in SEEDS:
    ...
    fig.suptitle(f"Seed robustness on GPT-2 LARGE (774M, block {LAYER}/36, {len(SEEDS)} seeds)")
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS, "30_seed_robustness_large.png"), dpi=120)
    plt.close(fig)
