...
### Seed robustness on Pythia-410m / GPT-NeoX (Experiment 28)

Experiments 26/27 put error bars on two GPT-2 *scales*, but the *architecture* axis (Qwen3 Exp 21, Pythia
Exp 24; the 81–94% band) was still single-seed. Pythia sits at the *low* end of that band (81% @α=8, below
both GPT-2 scales), so its position is exactly where a single-seed run is most in doubt. Experiment 28 puts the
same five-seed control on the **exact Experiment-24 Pythia-410m pipeline** — same DiffMean sentiment vector at
block 12/24 (`|v| = 3.29`), same 400-document Gaussian fit, same 300-document training set, same held-out
100-document eval, same 5.25M projection-preserving corrector at `d = 1024`, same recipe and `α ∼ U(0.5, 8)` —
at **five seeds (`0–4`)**. As in Experiments 26/27, `ΔLM_raw(α)` is seed-independent (computed once); only the
learned corrector varies; we report mean ± sample standard deviation of the fluency recovery. Seed 0 re-runs
Experiment 24 exactly and is the built-in reproducibility check.

### Seed robustness on Qwen3-1.7B (Experiment 29)

Experiments 26/27/28 give error bars on GPT-2 small, GPT-2 medium, and Pythia. The one remaining single-seed
point the study leans on is **Qwen3-1.7B (Experiment 21) — the *top* of the 81–94% architecture band**, whose
94% @α=8 is the largest single-seed recovery in the whole study and so the point where a lone seed is most in
doubt. Experiment 29 puts the same five-seed control on the **exact Experiment-21 Qwen3-1.7B pipeline** — same
DiffMean sentiment vector at block 14/28 (`|v| = 38.1`), same 400-document Gaussian fit, same 300-document
training set, same held-out 100-document eval, same 8.39M projection-preserving corrector at `d = 2048`, same
recipe and `α ∼ U(0.5, 8)` — at **five seeds (`0–4`)**. As in Experiments 26/27/28, `ΔLM_raw(α)` is
...
recovery on Qwen3 (94%) is even a touch higher than GPT-2 small's 84%. (This single-seed 94% is confirmed by a
five-seed control in Experiment 29: 94.8 ± 1.6% at α=8, a band entirely above every other model's.) And the
Experiment-2/3 decoupling holds a **fourth** time: the corrected activation sits **further** off the Gaussian
manifold than raw at **every** α (`D_M` learned > raw throughout, 122.2 vs 77.8 at α=8).
...
### Experiment 28 — the cross-architecture recovery is stable across seeds

![seed robustness on Pythia-410m](plots/28_seed_robustness_pythia.png)

Five independently-trained correctors (seeds 0–4), the exact Pythia-410m / GPT-NeoX pipeline, mean ± sample standard deviation:
...
**Observation.** At α=8 the five Pythia correctors recover **80.8 ± 1.6%** of raw steering's fluency damage
(range 78–82%), tightening to **81.7 ± 0.3%** at α=4. The corrected activation sits *further* off the Gaussian
manifold than raw at every seed (`D_M` 80.8 ± 6.6 vs raw 52.3 at α=8). Seed 0 reproduces Experiment 24 to the digit.

**Interpretation.** The recipe is **seed-stable on a third, non-GPT-2 architecture** — the corrector's advantage
over raw is many times its seed-to-seed spread at every strength that matters. Placing all three α=8 bands side
by side puts the architecture *ordering* on a controlled footing: Pythia's band `[79.2, 82.4]%` sits **entirely
below** GPT-2 medium's `[86.1, 90.5]%` (a genuine gap, not seed noise) but **overlaps** GPT-2 small's
`[81.3, 85.3]%`, so Pythia and small are within seed noise of each other at α=8. The 81–94% architecture band is
therefore real at its low end but should be read as three seed-controlled points, not a hard ranking of every pair.
...
*Done in Experiment 29.*

### Experiment 29 — the top-of-band recovery is stable across seeds, and Qwen3's edge is real

![seed robustness on Qwen3-1.7B](plots/29_seed_robustness_qwen.png)

Five independently-trained correctors (seeds 0–4), the exact Qwen3-1.7B pipeline, mean ± sample standard deviation:
...
**Observation.** At α=8 the five Qwen3 correctors recover **94.8 ± 1.6%** of raw steering's fluency damage
(range 92–96%), and at α≤4 they land *at or above* the unsteered baseline (recovery ≥ 100%, `ΔLM_learned`
slightly negative). The corrected activation sits *further* off the Gaussian manifold than raw at every seed
(`D_M` 123.3 ± 5.4 vs raw 77.8 at α=8). Seed 0 reproduces Experiment 21 to the digit.

**Interpretation.** The **top** of the architecture band is now seed-controlled, and it holds: Qwen3's α=8 band
`[93.2, 96.4]%` sits **entirely above** GPT-2 medium's `[86.1, 90.5]%`, GPT-2 small's `[81.3, 85.3]%`, and
Pythia's `[79.2, 82.4]%`. So across four seed-controlled models the ordering is Qwen3 > medium > {small ≈ Pythia},
and Qwen3's top-of-band 94% is a genuine effect rather than optimization luck. The seed axis now spans two model
scales and two architectures with no band-crossing ambiguity except the small/Pythia overlap.
...
**Next check.** *Done in Experiment 36* — the CONTENT class is refined by target-token frequency (an objective cut,
avoiding an unreliable out-of-context POS tagger), testing whether rare information-rich content words recover as
well as common ones.

### Experiment 36 — recovery is uniform across content-word frequency, so the pooled 84% is not carried by easy common words

![content-word frequency breakdown of the flagship recovery](plots/36_content_frequency.png)

Experiment 35 showed the CONTENT class as a whole recovers ~77.5% at α=8, but "content words" bundles common,
easy-to-predict nouns with rare, surprising ones. A part-of-speech split (noun/verb/adjective) needs an in-context
tagger that GPT-2's word-piece tokens do not support reliably, so we take an objective, fully-controlled cut
instead: split the CONTENT class by how **common** each target token is in the eval corpus. We reuse Experiment 35's
setup exactly (flagship Exp-3 corrector, GPT-2 small, block 6, sentiment `v`, seed 0; per-target-token NLL on the
same held-out 100 docs) and split CONTENT at the token-**weighted median** frequency, so the two buckets hold
~equal numbers of predicted tokens (2358 vs 2362). The cut lands at count 2: `CONTENT_COMMON` = content tokens seen
≥ 2× as a target, `CONTENT_RARE` = seen exactly once (hapax content tokens).

| content class | share | clean NLL | raw excess @α=8 | learned excess @α=8 | **recovery @α=8** | recovery @α=4 |
|---|---|---|---|---|---|---|
| CONTENT_COMMON (seen ≥2×) | 15% | 4.27 | +3.67 | +0.83 | **77.3%** | 83.8% |
| CONTENT_RARE (hapax) | 15% | 6.04 | +4.11 | +0.92 | **77.8%** | 81.4% |
| **pooled** | 100% | — | +2.78 | +0.44 | **84.3%** | 95.3% |

**Observation.** Rare (hapax) content tokens are much harder to predict even on clean text (clean NLL 6.04 vs 4.27
nats) and take slightly more absolute raw-steering damage (+4.11 vs +3.67 nats at α=8), yet the corrector recovers
them essentially identically to common content tokens — 77.8% vs 77.3% at α=8 (81.4% vs 83.8% at α=4). Pooled
recovery reproduces Experiment 3 / 34 / 35 to the digit (84.3% / 95.3%).

**Interpretation.** The fluency win is not concentrated on frequent, easy content words: the corrector buys back the
same fraction of raw's damage on the surprising, information-rich ones, which is where a broken steer would most
distort the model's predictions. Together with Experiments 34 (position) and 35 (type), this closes the
token-control axis — the pooled headline is not a pooling, a cheap-token, or a common-word artifact.

**Limitations.** One steering direction (sentiment), one model, one layer, seed 0. Frequency is a proxy for
"information-rich" but not identical to it; a proper part-of-speech or named-entity split would be finer, but the
near-equal recovery across a 6× clean-NLL gap already argues the corrector does not privilege easy tokens.

**Next check.** *Done in Experiment 37* — the last residual is the OTHER catch-all class, which Experiment 35 left
lumped; Experiment 37 decomposes it to check whether the pool's lift above the linguistic classes is trivial
punctuation.

### Experiment 37 — the pooled 84% is lifted above the whole-word linguistic recovery (~77%) by easy sub-word and punctuation tokens

![OTHER-class decomposition of the flagship recovery](plots/37_other_decomposition.png)

Experiments 35–36 established that recovery is uniform *within* whole words (FUNCTION 73.9%, CONTENT 77.5% at α=8,
flat across content-word frequency). But the pooled 84.3% sits *above* both, because a third catch-all class —
OTHER (subword continuation pieces + punctuation + digits) — recovers ~100% and, carrying large excess NLL, pulls
the token-weighted pool up. This experiment asks whether that near-100% OTHER recovery is trivial **punctuation** (a
cheap-token effect that would make the pooled headline flatter than the real linguistic recovery) or also holds on
genuine **subword** word-pieces. We reuse Experiment 35's setup exactly (flagship Exp-3 corrector, GPT-2 small,
block 6, sentiment `v`, seed 0; per-target-token NLL on the same held-out 100 docs) and split OTHER by whether the
decoded token contains a letter: `SUBWORD` (any alphabetic char — mid-word continuation pieces, 15,157 vocab types)
vs `PUNCT` (no alphabetic char — punctuation, digits, symbols, 2,996 types). FUNCTION and CONTENT are exactly
Experiment 35's classes, so the four-way pool reproduces the headline.
...
**Observation.** The near-complete OTHER recovery is not just punctuation: SUBWORD word-continuation pieces recover
91.7% at α=8 (118% at α=4), well above the two whole-word linguistic classes and close to PUNCT's 109.7% (>100% is
the usual ratio artifact — for punctuation the corrector's residual excess NLL is at or slightly below clean). The
whole-word linguistic-only pool (FUNCTION+CONTENT) recovers 76.8% at α=8; the pooled headline is 84.3%. Pooled
reproduces Experiment 3 / 34 / 35 to the digit.

**Interpretation.** The flagship 84% is a genuine but partly-inflated summary. The ~7-point gap between the pooled
84.3% and the whole-word linguistic 76.8% comes from two non-word token kinds — punctuation *and* subword pieces —
that the corrector fixes almost perfectly. So the honest figure for how much of raw's damage is recovered *on
meaning-bearing whole words specifically* is a still-strong ~77% at α=8; the pool is higher because the corrector is
strongest on the easiest (non-word) targets. This sharpens, without overturning, the token-control story.

**Limitations.** One steering direction (sentiment), one model, one layer, seed 0. The SUBWORD/PUNCT split is by a
simple has-a-letter rule on the decoded token, so a few mixed tokens (e.g. alphanumerics) are classed by that rule;
the linguistic-vs-non-word contrast is robust to those edge cases given the class sizes.

**Next check.** *Done in Experiment 38* — all recovery numbers (this axis included) are in cross-entropy nats;
Experiment 38 checks whether the recovery survives a metric the corrector never trained on: next-token accuracy.

### Experiment 38 — the recovery holds on next-token accuracy, so it is not a soft-metric (tail-mass) artifact

![Accuracy-metric control of the flagship recovery](plots/38_accuracy_metric.png)

Every recovery number in this report is measured in cross-entropy nats (`ΔLM`). A cross-entropy improvement could, in
principle, come entirely from re-shaping the probability *tail* while the model's actual argmax prediction stays
broken — which would make the headline a soft-metric artifact. This experiment scores the exact flagship pipeline
(GPT-2 small, block 6, sentiment `v`, seed 0 corrector) on a harder-to-game metric: **next-token top-1 (greedy)
accuracy** and **top-5 accuracy** on the same held-out 100 docs, for clean / raw-steered / learned-corrected
activations at matched projection `α|v|`. Top-k accuracy is the fraction of non-pad targets `y_t` in the model's
top-k prediction under patch `f`:

```math
\mathrm{acc}_k(f) = \frac{1}{N}\sum_{t} \mathbf{1}\!\left[\, y_t \in \mathrm{top}\text{-}k\, \mathrm{softmax}\big(\mathrm{LM}_{f}(x_{<t})\big) \,\right]
```

We report an **accuracy recovery**, the direct analogue of the `ΔLM` recovery — the fraction of raw steering's
accuracy *drop* the corrector buys back:

```math
\mathrm{acc\text{-}recovery}(\alpha) = \frac{\mathrm{acc}_{\text{learned}} - \mathrm{acc}_{\text{raw}}}{\mathrm{acc}_{\text{clean}} - \mathrm{acc}_{\text{raw}}}
```
...
**Observation.** Raw steering does not merely inflate cross-entropy; it crushes the model's argmax accuracy, from a
clean 0.356 down to 0.118 at α=8 (a 67% relative drop). The learned corrector restores top-1 accuracy to 0.297,
recovering 75% of that drop (88% at α=4, ≈100% at α≤2); top-5 recovery is similar (78% at α=8). The α=1 row reads
>100% for the usual ratio reason (raw's accuracy drop is near zero there).

**Interpretation.** Because top-1 accuracy depends only on which token is *most likely* — not on the tail
probability mass cross-entropy also rewards — the recovery is not a soft-metric artifact: the corrector genuinely
puts the right token back on top. The accuracy recovery runs slightly below the nats recovery (75% vs 84.3% at α=8),
which is expected — recovering the single argmax is a stricter bar than recovering average log-probability — but the
two broadly agree, so the headline holds under a metric the corrector was never trained on.

**Limitations.** One steering direction (sentiment), one model, one layer, seed 0; top-1/top-5 are one family of
hard metrics (a calibration or generation-quality metric could behave differently).

**Next check.** None material — the flagship recovery now holds in both nats and accuracy; the metric-control axis is
exhausted.
