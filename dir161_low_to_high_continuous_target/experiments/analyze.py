#!/usr/bin/env python3
"""
dir161 S4 — aggregate the probe, bootstrap the paired comparison, make figures.
...
Writes results/aggregate.json and the interpolation PNGs in plots/.
...
from common import (C_CLF, C_PRE, CVD, MLP, N_POINTS, PLOTS, RESULTS, Pdet,
...
KINDS = [('clf', 'classifier', C_CLF, '-', 'o'),
         ('pre', 'low-to-high predictor', C_PRE, '--', 's')]
...
def fig_training():
    ...
    for k, lbl, c, ls, mk in KINDS:
        ...
        for i, s in enumerate(SEEDS):
            ...
            st = np.array(h['step']); vl = np.array(h['val_loss'])
            j = int(np.argmin(vl[1:])) + 1
            ax.plot(st[1:], h['train_loss'][1:], color=CVD[0], ls='-', lw=1, alpha=.8,
                    label='train loss' if i == 0 else None)
            ax.plot(st[1:], vl[1:], color=CVD[1], ls='--', lw=1, alpha=.8,
                    label='val loss' if i == 0 else None)
    ...
    axes[2].set_xlabel('training step'); axes[2].set_ylim(0.98, 1.20)
    axes[2].set_ylabel('val loss / its minimum')
    axes[2].set_title('validation loss, rescaled (3 seeds)'); axes[2].legend(fontsize=7)
...
def fig_hand(P):
    meta = P[(0, 'clf')]
    fig, axes = plt.subplots(len(HAND), 3, figsize=(9.5, 2.3 * len(HAND)),
...
def fig_mean_curves(P):
    ...
    for c, (key, lname) in enumerate(LAYERS):
        ...
        for k, lbl, col, ls, mk in KINDS:
            ...
            ax.plot(ALPHA, m, color=col, ls=ls, lw=1.8, label=lbl, marker=mk,
                    markevery=12, ms=4)
...
def fig_paired(agg):
    ...
    for pane, (met, name) in enumerate([('lin_dev', 'linearity deviation'),
        ...
        for si, s in enumerate(SEEDS):
            ...
            ax.scatter(xs + .12 * (si - 1), v, color=CVD[3], marker='^', s=22, zorder=3,
                       label='per-seed mean' if si == 0 else None)
        ...
        ax.set_xticks(xs); ax.set_xticklabels([n for _, n in LAYERS], fontsize=8)
        ax.set_ylabel(f'classifier $-$ predictor\n({name})')
...
def fig_scatter(P):
    ...
    for c, (key, lname) in enumerate(LAYERS):
        ...
        y = np.mean([lin_dev(P[(s, 'pre')][key]) for s in SEEDS], 0)
        ...
        ax.text(.05, .93, f'predictor lower on {(y < x).mean()*100:.0f}% of pairs',
                transform=ax.transAxes, fontsize=7.5)
        if c == 0:
            ax.set_ylabel('predictor deviation'); ax.legend(fontsize=7, loc='lower right')
    fig.tight_layout(); fig.savefig(os.path.join(PLOTS, 'per_pair_scatter.png'))
    plt.close(fig)


def fig_path(device):
    """Model outputs along the 6->7 path: full 28x28 prediction, its removed-detail
    component, and the classifier's 10 logits."""
    ds = build_dataset()
    ...
    outs = {}
    for kind, n_out in [('pre', 784), ('clf', 10)]:
        ...
        with torch.no_grad():
            ...
            o, _ = m.forward_from(h1.reshape(-1, 200), 1)
            outs[kind] = o.reshape(N_POINTS, -1)
    det = Pdet(outs['pre']).cpu().numpy()
    full = outs['pre'].cpu().numpy()
    logit = outs['clf'].cpu().numpy()

    fig, axes = plt.subplots(3, 11, figsize=(11, 3.6))
    for j, t in enumerate(show):
        for r, (img, cm, vmin, vmax) in enumerate([
                (full[t].reshape(28, 28), 'gray', 0, 1),
                (det[t].reshape(28, 28), 'coolwarm', -.5, .5)]):
            ax = axes[r, j]
            ax.imshow(img, cmap=cm, vmin=vmin, vmax=vmax)
            ax.set_xticks([]); ax.set_yticks([]); ax.grid(False)
            if r == 0:
                ax.set_title(f'$\\alpha$={ALPHA[t]:.1f}', fontsize=7)
        ax = axes[2, j]
        ax.bar(range(10), logit[t], color=CVD[0])
        ax.set_ylim(-.4, 1.2); ax.set_xticks([]); ax.set_yticks([]); ax.grid(False)
        ax.set_xlabel(f'argmax {int(logit[t].argmax())}', fontsize=6.5, labelpad=1)
    for r, name in enumerate(['predictor\n$\\hat{y}(\\alpha)$ 28x28',
                              'detail\n$P(\\hat{y}(\\alpha))$',
                              'classifier\n10 outputs']):
        axes[r, 0].set_ylabel(name, fontsize=6.5)
    fig.suptitle(r'6 $\rightarrow$ 7 path, seed 0 (best-val checkpoints)', fontsize=9)
    fig.tight_layout(); fig.savefig(os.path.join(PLOTS, 'path_predictions.png'))
    plt.close(fig)
...
def fig_ckpt_control(P, Pf):
    ...
    series = [('classifier, best-val ckpt', P, 'clf', C_CLF, '-', 'o'),
              ...
              ('predictor, best-val ckpt', P, 'pre', C_PRE, '--', 's'),
              ('predictor, final step 30k', Pf, 'pre', CVD[3], (0, (3, 1, 1, 1)), 'D')]
    out = {}
    for c, (key, lname) in enumerate(LAYERS):
        ax = axes[c]; vals = {}
...
def main():
    ...
    for met, fn in [('lin_dev', lin_dev), ('max_jump', max_jump)]:
        ...
        for key, _ in LAYERS:
            ...
            r = np.mean([fn(P[(s, 'pre')][key]) for s in SEEDS], 0)
            lo, hi = boot_ci(c - r, rng)
            agg['pooled'][met][key] = {
                'clf_mean': float(c.mean()), 'pre_mean': float(r.mean()),
    ...
    for s in SEEDS:
        ...
        for met, fn in [('lin_dev', lin_dev), ('max_jump', max_jump)]:
            ...
            for key, _ in LAYERS:
                c, r = fn(P[(s, 'clf')][key]), fn(P[(s, 'pre')][key])
                lo, hi = boot_ci(c - r, rng)
    ...
    for key in ['f_h2', 'f_h3', 'f_out']:
        ...
        r = np.mean([lin_dev(P[(s, 'pre')][key]) for s in SEEDS], 0)
        lo, hi = boot_ci(c - r, rng)
        agg['frac_form'][key] = {'clf_mean': float(c.mean()), 'pre_mean': float(r.mean()),
                                 'diff_mean': float((c - r).mean()), 'ci': [lo, hi]}
    agg['n_pairs'] = int(P[(0, 'clf')]['d_h3'].shape[0])
    agg['seeds'] = SEEDS

    fig_training(); fig_hand(P); fig_mean_curves(P); fig_paired(agg)
    fig_scatter(P); fig_path(device)
