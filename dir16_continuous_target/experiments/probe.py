...
def main():
    ...
    for seed in SEEDS:
        ...
        for kind, n_out in [('clf', 10), ('reg', 49)]:
            ...
            m.load_state_dict(torch.load(os.path.join(d, f'{kind}.pt'),
                                         map_location=device))
            m.eval()
            for p in m.parameters():
                p.requires_grad = False
            rec = probe(m, ex_a, ex_b)
            np.savez_compressed(os.path.join(d, f'probe_{kind}.npz'), **rec, **meta)
            print(f'seed{seed} {kind}: d_h3 mean|d-a| = '
