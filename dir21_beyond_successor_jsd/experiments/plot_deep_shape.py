"""Figure: the same four probes read from the residual stream at four depths inside Pythia-1.4B.

One panel per block -- 0, 6, 12, 18. Each row is one target: the dot is the mean held-out Spearman
correlation over the 50 shared train/test splits, the bar its spread, the shaded strip the
50-permutation null, and the caret the ceiling that target's own measurement noise allows. Block 0 is
the same site pattern 44 already measured, refit here from hidden_states as a tie-check.

Writes plots/deep_shape.png.
"""
import json
...
def main():
    d = json.load(open(f"{RESULTS}/deep_shape.json"))
    blocks = d["blocks"]
    fig, axes = plt.subplots(1, len(blocks), figsize=(4.5 * len(blocks), 3.5),
                             sharex=True, sharey=True)
    for ax, b in zip(axes, blocks):
        title = (f"residual after block {b}" if b else
                 "residual after block 0  (tie-check)")
        panel(ax, d, f"block{b}", title)
    ...
    fig.savefig(f"{PLOTS}/deep_shape.png", dpi=170)
    plt.close(fig)
    print(f"wrote {PLOTS}/deep_shape.png")
